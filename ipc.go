...
// processes to communicate with and and manage the lifetime of a hcashd child
...
// message, shutdown begins.  This prevents hcashd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			hcashdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child hcashd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	hcashdLog.Errorf("Failed to write to pipe: %v", err)
}
