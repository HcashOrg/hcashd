...
//
// In addition to the main Hypercash network, which is intended for the transfer
...
// wire.HypercashNet using ParamsForNet, but be aware that this usage is
...
//  )
//
//  var testnet = flag.Bool("testnet", false, "operate on the testnet Hypercash network")
//
...
//          addr, err := hcashutil.NewAddressPubKeyHash(pubKeyHash, chainParams)
//          if err != nil {
//                  log.Fatal(err)
//          }
//          fmt.Println(addr)
//  }
//
// If an application does not use one of the three standard Hypercash networks,
