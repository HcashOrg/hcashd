...
func (m *CPUMiner) submitBlock(block *hcashutil.Block) bool {
	...
	if blockchain.HashToBig(block.Hash()).Cmp(blockchain.CompactToBig(block.MsgBlock().Header.Bits)) < 0 {
		...
		excoinbaseTxOuts := block.MsgBlock().Transactions[0].TxOut
		for i, out := range excoinbaseTxOuts {
			if i == 0 {
				//Tax
				continue
			}
		...
		coinbaseTxOuts := block.MsgBlock().Transactions[1].TxOut
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"height %v, keyheight %v, amount %v ) PrevKeyHash : %v", block.Hash(),block.Height(), block.MsgBlock().Header.KeyHeight,
		hcashutil.Amount(coinbaseTxGenerated) , block.MsgBlock().Header.PrevKeyBlock)
	return true
}
...
func (m *CPUMiner) solveBlock(template *BlockTemplate, ticker *time.Ticker,
	...
	//microTargetDifficulty := targetDifficulty.Mul(targetDifficulty, big.NewInt(int64(activeNetParams.DifficultyRate)))
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		err := UpdateExtraNonce(msgBlock, uint32(blockHeight), blockKeyHeight, ens)
		if err != nil {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				if (lastTxUpdate != m.txSource.LastUpdated() &&
					...
					time.Now().After(lastGenerated.Add(5*time.Second)) {
			...
			if blockchain.HashToBig(&hash).Cmp(targetDifficulty) <= 0 {
				...
				if blockchain.HashToBig(&hash).Cmp(hardTargetDifficulty) > 0 {
					...
					msgBlock.STransactions = stransactions
				}
				return true
			}
		}
...
out:
	for {
		...
		// Hacks to make hcash work with Hypercash PoC (simnet only)
		// TODO Remove before production.
		if cfg.SimNet {
			_, curHeight, _ := m.server.blockManager.chainState.Best()
