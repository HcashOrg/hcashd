...
func (b *blockManager) checkBlockForHiddenVotes(block *hcashutil.Block) {
	...
	extraCoinbase, err := createExtraCoinbaseTx(b.chain.FetchSubsidyCache(),
	...
	template.Block.Transactions[0] = extraCoinbase.MsgTx()

	coinbase, err := createCoinbaseTx(template.Block.Transactions[0].TxIn[0].SignatureScript,
		opReturnPkScript,
		cfg.miningAddrs[rand.Intn(len(cfg.miningAddrs))])
	...
	for i, mtx := range template.Block.Transactions {
		...
		tx := hcashutil.NewTx(mtx)
		updatedTxTreeRegular = append(updatedTxTreeRegular, tx)
	}
	merkles := blockchain.BuildMerkleTreeStore(updatedTxTreeRegular, false)
	template.Block.Header.StakeRoot = *merkles[len(merkles)-1]
	smerkles := blockchain.BuildMerkleTreeStore(updatedTxTreeStake, false)
...
func (b *blockManager) handleBlockMsg(bmsg *blockMsg) {
	...
	if isOrphan {
		...
		keyHeightUpdate = int64(header.KeyHeight)
		...
		_, _, bestKeyHeight := b.chainState.Best()
		blockKeyHeight := int64(bmsg.block.MsgBlock().Header.KeyHeight)
		tooOldForLotteryData := blockKeyHeight <=
			(bestKeyHeight - maxLotteryDataBlockDelta)
		if !tooOldForLotteryData {
			...
			// checkpoint.
			winningTickets, _, _, err := b.chain.LotteryDataForBlock(blockHash)
			realKeyHeight := bmsg.block.MsgBlock().Header.KeyHeight

			isKeyBlock := false
			if blockchain.HashToBig(bmsg.block.Hash()).Cmp(blockchain.CompactToBig(bmsg.block.MsgBlock().Header.Bits)) <= 0{
				realKeyHeight++
				isKeyBlock = true
			}
			if err != nil && int64(realKeyHeight) + 1 >=
				b.server.chainParams.StakeValidationHeight {
			...
			if isKeyBlock && int64(realKeyHeight + 1)>= b.server.chainParams.StakeValidationHeight{
				...
				if !beenNotified && r != nil &&
					int64(bmsg.block.MsgBlock().Header.Height) > b.server.chainParams.LatestCheckpointHeight() {
		...
		if onMainChain {
			...
			if b.AggressiveMining && (int64(bmsg.block.MsgBlock().Header.KeyHeight) + 1) >= svl {
	...
	if blkHashUpdate != nil && heightUpdate != 0 && keyHeightUpdate != 0 {
		bmsg.peer.UpdateLastBlockHeight(heightUpdate)
		bmsg.peer.UpdateLastKeyBlockHeight(keyHeightUpdate)
		if isOrphan || b.current() {
			go b.server.UpdatePeerHeights(blkHashUpdate, heightUpdate, keyHeightUpdate,
				bmsg.peer)
		}
	}
...
out:
	for {
		...
		case m := <-b.msgChan:
			...
			case getKeyGenerationMsg:
				g, err := b.chain.GetKeyGeneration(msg.hash)
				msg.reply <- getKeyGenerationResponse{
					hashes: g,
					err:    err,
				}
			case getMatchedDescendantsMsg:
				g, err := b.chain.GetMatchedDescendants(msg.hash, msg.voteResult)
				msg.reply <- getMatchedDescendantsResponse{
					hashes: g,
					err:  err,
				}
			...
			case processBlockMsg:
				...
				_, _, bestKeyHeight := b.chainState.Best()
				blockKeyHeight := int64(msg.block.MsgBlock().Header.KeyHeight)
				tooOldForLotteryData := blockKeyHeight <=
					(bestKeyHeight - maxLotteryDataBlockDelta)
				if !isOrphan && !tooOldForLotteryData {
					...
					b.lotteryDataBroadcastMutex.Unlock()
					winningTickets, _, _, err := b.chain.LotteryDataForBlock(msg.block.Hash())
					realKeyHeight := msg.block.MsgBlock().Header.KeyHeight

					isKeyBlock := false
					if blockchain.HashToBig(msg.block.Hash()).Cmp(blockchain.CompactToBig(msg.block.MsgBlock().Header.Bits)) <= 0{
						realKeyHeight++
						isKeyBlock = true
					}
					if err != nil && int64(realKeyHeight) + 1 >=
					...
					// height.
					r := b.server.rpcServer
					if r != nil && isKeyBlock &&!isOrphan && !beenNotified &&
						(int64(realKeyHeight) + 1 >=
							b.server.chainParams.StakeValidationHeight) &&
						(msg.block.Height() >
			...
			case processTransactionMsg:
				acceptedTxs, err := b.server.txMemPool.ProcessTransaction(b.chain, msg.tx,
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		tooOldForLotteryData := blockKeyHeight <= (bestKeyHeight - maxLotteryDataBlockDelta)
		realKeyHeight := block.MsgBlock().Header.KeyHeight
		isKeyBlock := false
		if blockchain.HashToBig(block.Hash()).Cmp(blockchain.CompactToBig(block.MsgBlock().Header.Bits)) <= 0{
			realKeyHeight++
			isKeyBlock = true
		}
		if int64(realKeyHeight) + 1 >=
			b.server.chainParams.StakeValidationHeight &&
	...
	case blockchain.NTBlockConnected:
		blockSlice, ok := notification.Data.([]*hcashutil.Block)
		if !ok {
		...
		if !txTreeRegularValid && isParentKeyBlock{
			for _, tx := range parentBlock.Transactions()[1:] {
				_, err := b.server.txMemPool.MaybeAcceptTransaction(b.chain, tx, false,
					true)
				if err != nil {
		...
		if isKeyBlock{
			for _, tx := range block.Transactions()[2:] {
				...
				acceptedTxs := b.server.txMemPool.ProcessOrphans(b.chain, tx.Hash())
				b.server.AnnounceNewTransactions(acceptedTxs)
			}
		}else {
		...
		if isKeyBlock {
			for _, stx := range block.STransactions()[0:] {
				...
				acceptedTxs := b.server.txMemPool.ProcessOrphans(b.chain, stx.Hash())
				b.server.AnnounceNewTransactions(acceptedTxs)
			}
		}
		...
		if r := b.server.rpcServer; r != nil {
			...
			if txTreeRegularValid || !isParentKeyBlock{
				if isParentKeyBlock{
					for _, tx := range parentBlock.Transactions()[2:] {
	...
	case blockchain.NTBlockDisconnected:
		...
		if !txTreeRegularValid && isParentKeyBlock{
			for _, tx := range parentBlock.Transactions()[1:] {
				...
				b.server.txMemPool.RemoveDoubleSpends(tx)
				b.server.txMemPool.RemoveOrphan(tx.Hash())
				b.server.txMemPool.ProcessOrphans(b.chain, tx.Hash())
			}
		}
...
// queue.
func (b *blockManager) QueueTx(tx *hcashutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *hcashutil.Block, sp *serverPeer) {
...
func (b *blockManager) GetKeyGeneration(h chainhash.Hash) ([]chainhash.Hash, error) {
	reply := make(chan getKeyGenerationResponse)
	b.msgChan <- getKeyGenerationMsg{hash: h, reply: reply}
	response := <-reply
	return response.hashes, response.err
}
...
func (b *blockManager) GetTopBlockFromChain() (*hcashutil.Block, error) {
...
func (b *blockManager) ProcessBlock(block *hcashutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *hcashutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*hcashutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (hcashutil.Amount, error) {
...
// newBlockManager returns a new hypercash block manager.
...
func newBlockManager(s *server, indexManager blockchain.IndexManager) (*blockManager, error) {
	...
	curPrevHash, curPrevKeyHash := bm.chain.BestPrevHash()
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		hcashdLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB() (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		hcashdLog.Infof("Creating block database in memory.")
	...
	hcashdLog.Infof("Loading block database from '%s'", dbPath)
	...
	hcashdLog.Info("Block database loaded")
	return db, nil
}
