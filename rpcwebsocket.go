...
func (*wsNotificationManager) notifyStakeDifficulty(
	...
	// Notify interested websocket clients about the connected block.
	ntfn := hcashjson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *hcashutil.Tx) {
	...
	ntfn := hcashjson.NewTxAcceptedNtfn(txHashStr,
		hcashutil.Amount(amount).ToCoin())
	marshalledJSON, err := hcashjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *hcashjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			rawTx, err := createTxRawResult(net, mtx, txHashStr,
				wire.NullBlockIndex, nil, "", 0, 0, 0)
			if err != nil {
				return
			}

			verboseNtfn = hcashjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = hcashjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *hcashutil.Tx,
	clients map[chan struct{}]*wsClient) {
	...
	if len(clientsToNotify) != 0 {
		n := hcashjson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := hcashjson.MarshalCmd(nil, n)
		if err != nil {
...
out:
	for {
		...
		switch authCmd, ok := cmd.cmd.(*hcashjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &hcashjson.RPCError{
					Code:    hcashjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hcashjson.HelpCmd)
	if !ok {
		return nil, hcashjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*hcashjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &hcashjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hcashjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, hcashjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *hcashutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*hcashjson.RescanCmd)
	if !ok {
		return nil, hcashjson.ErrRPCInternal
	}
	...
	if filter == nil {
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := hcashjson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]hcashjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &hcashjson.RPCError{
				Code:    hcashjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, hcashjson.RescannedBlock{
	...
	return &hcashjson.RescanResult{DiscoveredData: discoveredData}, nil
}
