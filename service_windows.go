...
const (
	// svcName is the name of hcashd service.
	svcName = "hcashdsvc"
	...
	svcDisplayName = "Hcashd Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the hypercash block " +
...
// logServiceStartOfDay logs information about hcashd when the main server has
...
// hcashdService houses the main service handler which handles all service
// updates and launching hcashdMain.
type hcashdService struct{}
...
// long-running hcashdMain (which is the real meat of hcashd), handles service
// change requests, and notifies the service control manager of changes.
func (s *hcashdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start hcashdMain in a separate goroutine so the service can start
...
// installService attempts to install the hcashd service.  Typically this should
...
// removeService attempts to uninstall the hcashd service.  Typically this should
...
// startService attempts to start the hcashd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &hcashdService{})
	if err != nil {
