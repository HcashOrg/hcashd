...
import (
	...
	"github.com/HcashOrg/hcashd/blockchain/indexers"
...
const (
	...
	MinHighPriority = hcashutil.AtomsPerCoin * 144.0 / 250
	...
	// HCASH/kB, this results in a maximum allowed high fee of 1 HCASH/kB.
...
type Config struct {
	...
	FetchUtxoView func(*hcashutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*hcashutil.Block, error)
	...
	// BestKeyHeight defines the function to use to access the keyblock height of
	// the current best chain.
...
type Policy struct {
	...
	MinRelayTxFee hcashutil.Amount
...
type TxPool struct {
	...
	orphans       map[chainhash.Hash]*hcashutil.Tx
	orphansByPrev map[chainhash.Hash]map[chainhash.Hash]*hcashutil.Tx
	addrindex     map[string]map[chainhash.Hash]struct{} // maps address to txs
	outpoints     map[wire.OutPoint]*hcashutil.Tx
...
func (mp *TxPool) insertVote(ssgen *hcashutil.Tx) error {
	voteHash := ssgen.Hash()
	msgTx := ssgen.MsgTx()
	ticketHash := &msgTx.TxIn[1].PreviousOutPoint.Hash

	// Get the block it is voting on; here we're agnostic of height.
	blockHash, blockHeight, blockKeyHeight, err := stake.SSGenBlockVotedOn(msgTx)
	if err != nil {
		return err
	}
	...
	vote := hcashutil.IsFlagSet16(voteBits, hcashutil.BlockValid)
	mp.votes[blockHash] = append(vts, &VoteTx{*voteHash, *ticketHash, vote})

	log.Debugf("Accepted vote %v for block hash %v (height %v, keyHeight %v), voting "+
		"%v on the transaction tree", voteHash, blockHash, blockHeight, blockKeyHeight,
		vote)
...
// TODO Pruning of the votes map HYPERCASH
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *hcashutil.Tx) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if _, exists := mp.orphansByPrev[originTxHash]; !exists {
			mp.orphansByPrev[originTxHash] =
				make(map[chainhash.Hash]*hcashutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *hcashutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *hcashutil.Tx, removeRedeemers bool) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *hcashutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint,
	tx *hcashutil.Tx, txType stake.TxType, height int64, fee int64) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *hcashutil.Tx, txType stake.TxType) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *hcashutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash, includeRecentBlock bool) (*hcashutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
	...
	// For Hypercash, the latest block is considered "unconfirmed"
...
// It should also set the hcashutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(chain *blockchain.BlockChain, tx *hcashutil.Tx, isNew, rateLimit, allowHighFees bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
	...
	if txType == stake.TxTypeSSGen {
		_, _, voteKeyHeight, err := stake.SSGenBlockVotedOn(msgTx)
		if err != nil {
			return nil, err
		}

		if (int64(voteKeyHeight) < nextBlockKeyHeight-maximumVoteAgeDelta) &&
			...
			str := fmt.Sprintf("transaction %v votes on old "+
				...
				tx.Hash(), voteKeyHeight, nextBlockKeyHeight-maximumVoteAgeDelta)
			return nil, txRuleError(wire.RejectNonstandard, str)
		}
	}
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(chain *blockchain.BlockChain, tx *hcashutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	hashes, err := mp.maybeAcceptTransaction(chain, tx, isNew, rateLimit, true)
	mp.mtx.Unlock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(chain *blockchain.BlockChain, hash *chainhash.Hash) []*hcashutil.Tx {
	var acceptedTxns []*hcashutil.Tx
	...
	for processHashes.Len() > 0 {
		...
		for _, tx := range orphans {
			...
			// transaction pool.
			missingParents, err := mp.maybeAcceptTransaction(chain, tx,
				true, true, true)
			if err != nil {
...
func (mp *TxPool) ProcessOrphans(chain *blockchain.BlockChain, hash *chainhash.Hash) []*hcashutil.Tx {
	mp.mtx.Lock()
	acceptedTxns := mp.processOrphans(chain, hash)
	mp.mtx.Unlock()
	return acceptedTxns
}
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(chain *blockchain.BlockChain, tx *hcashutil.Tx, allowOrphan, rateLimit, allowHighFees bool) ([]*hcashutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
	var err error
	...
	var missingParents []*chainhash.Hash
	missingParents, err = mp.maybeAcceptTransaction(chain, tx, true, rateLimit,
		allowHighFees)
	if err != nil {
		return nil, err
	}
	...
	if len(missingParents) == 0 {
		...
		newTxs := mp.processOrphans(chain, tx.Hash())
		acceptedTxs := make([]*hcashutil.Tx, len(newTxs)+1)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose(filterType *stake.TxType) map[string]*hcashjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*hcashjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &hcashjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Fee:              hcashutil.Amount(desc.Fee).ToCoin(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphans:       make(map[chainhash.Hash]*hcashutil.Tx),
		orphansByPrev: make(map[chainhash.Hash]map[chainhash.Hash]*hcashutil.Tx),
		outpoints:     make(map[wire.OutPoint]*hcashutil.Tx),
