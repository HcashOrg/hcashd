...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*hcashutil.Block
	currentHash   chainhash.Hash
	currentHeight int64
}
// FakeChain returns a chain that is usable for syntetic tests.
func FakeChain() *blockchain.BlockChain {
...
func (s *fakeChain) FetchUtxoView(tx *hcashutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*hcashutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *hcashutil.Block) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *hcashutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   hcashutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*hcashutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*hcashutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput hcashutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*hcashutil.Tx, error) {
	txChain := make([]*hcashutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, hcashutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := hcashutil.NewAddressSecpPubKey(pubKeyBytes,
		chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	chain := &fakeChain{
		...
		blocks: make(map[chainhash.Hash]*hcashutil.Block),
	}
	harness := poolHarness{
		signKey:     signKey,
		payAddr:     payAddr,
		...
		txPool: New(&Config{
			...
			BestRealKeyHeight:   chain.BestHeight,
	...
	coinbase, err := harness.CreateCoinbaseTx(0, numOutputs)
	if err != nil {
		return nil, nil, err
	}
	harness.chain.utxos.AddTxOuts(coinbase, /*curHeight+1*/0, wire.NullBlockIndex)
	for i := uint32(0); i < numOutputs; i++ {
...
func TestSimpleOrphanChain(t *testing.T) {
	...
	for _, tx := range chainedTxns[1 : maxOrphans+1] {
		...
		acceptedTxns, err := harness.txPool.ProcessTransaction(bc, tx, true,
			false, true)
		if err != nil {
	...
	acceptedTxns, err := harness.txPool.ProcessTransaction(bc, chainedTxns[0],
		false, false, true)
	if err != nil {
		t.Fatalf("ProcessTransaction: failed to accept valid "+
			"orphan %v", err)
...
func TestOrphanReject(t *testing.T) {
	...
	for _, tx := range chainedTxns[1:] {
		...
		acceptedTxns, err := harness.txPool.ProcessTransaction(bc, tx, false,
			false, true)
		if err == nil {
