...
// newAddressPubKey returns a new hcashutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) hcashutil.Address {
	pubkey, err := chainec.Secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := hcashutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new hcashutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) hcashutil.Address {
	addr, err := hcashutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
		secp)
	if err != nil {
...
// newAddressScriptHash returns a new hcashutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) hcashutil.Address {
	addr, err := hcashutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the hcashutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the hcashutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := hcashutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := hcashutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := hcashutil.NewAddressSecpPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := hcashutil.NewAddressSecpPubKey(decodeHex("03b0bd"+
	...
	tests := []struct {
		...
		// pay-to-pubkey address on mainnet. for hypercash this would
		// be uncompressed, but standard for hypercash is 33 byte
		...
		{(*hcashutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*hcashutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*hcashutil.AddressSecpPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := hcashutil.NewAddressSecpPubKey(decodeHex("02192d7"+
	...
	tests := []struct {
		keys      []*hcashutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*hcashutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		{
			// By default compressed pubkeys are used in Hypercash.
			[]*hcashutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*hcashutil.AddressSecpPubKey),
			},
			1,
