...
func mkGetKey(keys map[string]addressToKey) txscript.KeyDB {
	if keys == nil {
		return txscript.KeyClosure(func(addr hcashutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) txscript.ScriptDB {
	if scripts == nil {
		return txscript.ScriptClosure(func(addr hcashutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return txscript.ScriptClosure(func(addr hcashutil.Address) ([]byte,
		error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := hcashutil.NewAddressPubKeyHash(
					hcashutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := hcashutil.NewAddressPubKeyHash(
				hcashutil.Hash160(pkBytes), testingParams, secp)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
				break
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := hcashutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case edwards:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = hcashutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case secSchnorr:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = hcashutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := hcashutil.NewAddressPubKeyHash(
					hcashutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := hcashutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := hcashutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := hcashutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := txscript.MultiSigScript(
				[]*hcashutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
var (
	...
	thisAddressUnc, _ = hcashutil.NewAddressPubKeyHash(
		hcashutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, secp)
	uncompressedPkScript, _ = txscript.PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = hcashutil.NewAddressPubKeyHash(
		hcashutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, secp)
