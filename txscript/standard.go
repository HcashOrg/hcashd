...
func PayToSStx(addr hcashutil.Address) ([]byte, error) {
	...
	if scriptType == PubkeyHashAltTy {
		return NewScriptBuilder().AddOp(OP_SSGEN).AddOp(OP_DUP).
			AddOp(OP_HASH160).AddData(hash).AddOp(OP_EQUALVERIFY).
			AddData(altData).AddOp(OP_CHECKSIG).Script()
	}
...
// an SStx.
func GenerateSStxAddrPush(addr hcashutil.Address, amount hcashutil.Amount,
...
func GenerateSSGenBlockRef(blockHash chainhash.Hash, height uint32, keyHeight uint32) ([]byte,
	error) {
	// Prefix
	dataPushes := []byte{
		0x6a, // OP_RETURN
	...
	// Serialize the block hash and height, keyHeight
	blockHeightAndKeyHeightBytes := make([]byte, 8)
	binary.LittleEndian.PutUint32(blockHeightAndKeyHeightBytes, height)

	binary.LittleEndian.PutUint32(blockHeightAndKeyHeightBytes[4:], keyHeight)

	blockData := append(blockHash[:], blockHeightAndKeyHeightBytes...)
...
// specified address.
func PayToAddrScript(addr hcashutil.Address) ([]byte, error) {
	switch addr := addr.(type) {
	case *hcashutil.AddressPubKeyHash:
		if addr == nil {
			return nil, ErrUnsupportedAddress
...
func MultiSigScript(pubkeys []*hcashutil.AddressSecpPubKey, nrequired int) ([]byte,
	error) {
...
func ExtractPkScriptAddrs(version uint16, pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []hcashutil.Address, int, error) {
	if version != DefaultScriptVersion {
		return NonStandardTy, nil, 0, fmt.Errorf("invalid script version")
	}

	var addrs []hcashutil.Address
	...
	case PubKeyTy:
		...
		if err == nil {
			addr, err := hcashutil.NewAddressSecpPubKeyCompressed(pk, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	case PubkeyAltTy:
		...
		switch suite {
		case chainec.ECTypeEdwards:
			addr, err = hcashutil.NewAddressEdwardsPubKey(pops[0].data,
				chainParams)
		case chainec.ECTypeSecSchnorr:
	...
	case StakeSubmissionTy:
		...
		var localAddrs []hcashutil.Address
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := hcashutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]hcashutil.Address, 0, numPubKeys)
