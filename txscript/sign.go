...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []hcashutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
	// No need to add dummy in Hypercash.
...
func handleStakeOutSign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	addresses []hcashutil.Address, class ScriptClass, subClass ScriptClass,
	nrequired int) ([]byte, ScriptClass, []hcashutil.Address, int, error) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []hcashutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []hcashutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []hcashutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		if err != nil {
			// Hypercash -- is this the right handling for SIGHASH_SINGLE error ?
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*hcashutil.AddressSecpPubKey)
...
type KeyDB interface {
	GetKey(hcashutil.Address) (chainec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(hcashutil.Address) (chainec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address hcashutil.Address) (chainec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(hcashutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(hcashutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address hcashutil.Address) ([]byte, error) {
	return sc(address)
}
