...
const DefaultUserAgent = "/hcashwire:0.3.0/"

// MsgVersion implements the Message interface and represents a hypercash version
...
type MsgVersion struct {
	...
	// Last key block seen by the generator of the version message
	LastKeyBlock int32
...
func (msg *MsgVersion) BtcDecode(r io.Reader, pver uint32) error {
	...
	if buf.Len() > 0 {
		err = readElement(buf, &msg.LastKeyBlock)
		if err != nil {
			return err
		}
...
func (msg *MsgVersion) BtcEncode(w io.Writer, pver uint32) error {
	...
	err = writeElement(w, msg.LastKeyBlock)
	if err != nil {
		return err
	}
...
// NewMsgVersion returns a new hypercash version message that conforms to the
// Message interface using the passed parameters and defaults for the remaining
// fields.
func NewMsgVersion(me *NetAddress, you *NetAddress, nonce uint64,
	lastBlock int32, lastKeyBlock int32) *MsgVersion {
...
// and local address from conn and returns a new hypercash version message that
// conforms to the Message interface.  See NewMsgVersion.
func NewMsgVersionFromConn(conn net.Conn, nonce uint64,
	lastBlock int32, lastKeyBlock int32) (*MsgVersion, error) {
	...
	return NewMsgVersion(lna, rna, nonce, lastBlock, lastKeyBlock), nil
}
