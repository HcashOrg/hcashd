...
For the complete details of the hypercash protocol, see the official wiki entry
...
supported hypercash messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Hypercash Message Overview

The hypercash protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which hypercash network it is a part of, its type, how big it is, and a checksum
to verify validity.  All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for hypercash messages named
...
of the currently supported hypercash messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using hypercash encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the hypercash messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official hypercash protocol wiki entry at
...
and write hypercash messages.  The following sections provide a quick overview of
...
The hypercash network is a magic number which is used to identify the start of a
message and which hypercash network the message applies to.  This package provides
...
As discussed in the hypercash message overview section, this package reads
and writes hypercash messages using a generic interface named Message.  In
...
In order to unmarshall hypercash messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a hypercash peer.  Example syntax is:

	// Reads and validates the next hypercash message from conn using the
	// protocol version pver and the hypercash network btcnet.  The returns
...
In order to marshall hypercash messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a hypercash peer.  Example syntax to request addresses
from a remote peer is:

	// Create a new getaddr hypercash message.
	msg := wire.NewMsgGetAddr()

	// Writes a hypercash message msg to conn using the protocol version
	// pver, and the hypercash network btcnet.  The return is a possible
	// error.
