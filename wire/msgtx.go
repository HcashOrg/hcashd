...
// OutPoint defines a hypercash data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new hypercash transaction outpoint point with the
...
// TxIn defines a hypercash transaction input.
type TxIn struct {
	// Non-witness
...
// NewTxIn returns a new hypercash transaction input with the provided
...
// TxOut defines a hypercash transaction output.
type TxOut struct {
	Value    int64
	Version  uint16
	PkScript []byte
...
// NewTxOut returns a new hypercash transaction output with the provided
...
// MsgTx implements the Message interface and represents a hypercash tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Hypercash
...
// in that BtcDecode decodes from the Hypercash wire protocol as it was sent
...
// encodes the transaction to the hypercash wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new hypercash tx message that conforms to the Message
...
// WriteOutPoint encodes op to the hypercash protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefixs encodes ti to the hypercash protocol encoding for a transaction
...
// writeTxWitness encodes ti to the hypercash protocol encoding for a transaction
...
// writeTxInWitnessSigning encodes ti to the hypercash protocol encoding for a
...
// writeTxInWitnessValueSigning encodes ti to the hypercash protocol encoding for a
...
// writeTxOut encodes to into the hypercash protocol encoding for a transaction
// output (TxOut) to w.
