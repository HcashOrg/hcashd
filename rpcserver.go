...
var (
	...
	// data.
	gbtCoinbaseAux = &hcashjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &hcashjson.RPCError{
	...
	ErrRPCNoWallet = &hcashjson.RPCError{
		Code:    hcashjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which hcashd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of hcashwallet.
...
// not needed.
func rpcInternalError(errStr, context string) *hcashjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return hcashjson.NewRPCError(hcashjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *hcashjson.RPCError {
	return hcashjson.NewRPCError(hcashjson.ErrRPCMisc, message)
}
...
// These commands will be implemented in hcashwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > hcashutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, hcashutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := hcashutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := hcashutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := hcashutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			hcashutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > hcashutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, hcashutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = hcashutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.CreateRawSSGenTxCmd)
	...
	blockHash, curHeight, curKeyHeight := s.server.blockManager.chainState.Best()
	stakeVoteSubsidy := blockchain.CalcStakeVoteSubsidy(
		s.chain.FetchSubsidyCache(), curKeyHeight, activeNetParams.Params)
	...
	blockRefScript, err := txscript.GenerateSSGenBlockRef(*blockHash,
		uint32(curHeight), uint32(curKeyHeight))
	if err != nil {
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > hcashutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.CreateRawSSRtxCmd)
	...
	var feeAmt hcashutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = hcashutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > hcashutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, hcashutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []hcashjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]hcashjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = hcashutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &hcashjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []hcashjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]hcashjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var reqSigs int
		var commitAmt *hcashutil.Amount
		...
		var vout hcashjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = hcashutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = hcashjson.Float64(commitAmt.ToCoin())
		}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.DecodeRawTransactionCmd)
	...
	txReply := hcashjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.DecodeScriptCmd)
	...
	p2sh, err := hcashutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := hcashjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.EstimateStakeDiffCmd)
	...
	_, bestHeight, _ := s.server.blockManager.chainState.Best()
	...
	if c.Tickets != nil {
		...
		userEstFlt := hcashutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &hcashjson.EstimateStakeDiffResult{
		Min:      hcashutil.Amount(min).ToCoin(),
		Max:      hcashutil.Amount(max).ToCoin(),
		Expected: hcashutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hcashjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := hcashutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hcashjson.ExistsAddressesCmd)
	addresses := make([]hcashutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := hcashutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ExistsMissedTicketsCmd)

	hashes, err := hcashjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetAddedNodeInfoCmd)

	// Retrieve a list of persistent (added) peers from the hypercash server
	...
	results := make([]*hcashjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result hcashjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = hcashjson.Bool(peer.Connected())
		...
		addrs := make([]hcashjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr hcashjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]hcashjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]hcashjson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetBlockHashCmd)
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetBlockHeaderCmd)
	...
	blockHeaderReply := hcashjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: confirmations,
		Version:       blockHeader.Version,
		PreviousHash:  blockHeader.PrevBlock.String(),
		...
		PoolSize:      blockHeader.PoolSize,
		Bits:          strconv.FormatInt(int64(blockHeader.Bits), 16),
		SBits:         hcashutil.Amount(blockHeader.SBits).ToCoin(),
...
func handleGetBlockKeyHeight(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetBlockKeyHeightCmd)
	height := c.Height
	keyHeight, err := s.chain.GetBlockKeyHeight(height)
	return keyHeight, err
}
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetBlockSubsidyCmd)
	...
	dev := blockchain.CalcBlockTaxSubsidy(cache, keyheight, voters,
		s.server.chainParams)
	pos := blockchain.CalcStakeVoteSubsidy(cache, keyheight,
		s.server.chainParams) * int64(voters)
	pow := blockchain.CalcBlockWorkSubsidy(cache, keyheight, voters,
		s.server.chainParams)
	total := dev + pos + pow

	rep := hcashjson.GetBlockSubsidyResult{
		Developer: dev,
		PoS:       pos,
		PoW:       pow,
		Total:     total,
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	latestHash, _ ,_:= s.server.blockManager.chainState.Best()
...
// state as a hcashjson.GetBlockTemplateResult that is ready to be encoded to
// JSON and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*hcashjson.GetBlockTemplateResult, error) {
	...
	newestBlock, _ , _:= bm.chainState.Best()
	if newestBlock == nil {
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCBestBlockHash,
	...
	for i, tx := range msgBlock.Transactions {
		...
		if !recalculateFeesAndSigsOps {
			...
			txU := hcashutil.NewTx(tx)
			isValid := hcashutil.IsFlagSet16(
		...
		resultTx := hcashjson.GetBlockTemplateResultTx{
	...
	reply := hcashjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *hcashjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !cfg.SimNet && s.server.ConnectedCount() == 0 {
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCClientNotConnected,
			Message: "Hypercash is not connected",
		}
	}

	// No point in generating or accepting work before the chain is synced.
	_, currentHeight ,_:= s.server.blockManager.chainState.Best()
	if currentHeight != 0 && !s.server.blockManager.IsCurrent() {
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCClientInInitialDownload,
			Message: "Hypercash is downloading blocks...",
		}
	}
...
// reasons.
// TODO Hypercash pop in the new errors from blockchain cj
...
func handleGetBlockTemplateProposal(s *rpcServer, request *hcashjson.TemplateRequest) (interface{}, error) {
	...
	expectedPrevHash, _, _:= s.server.blockManager.chainState.Best()
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hcashjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetHeadersCmd)
	blockLocators, err := hcashjson.DecodeConcatenatedHashes(c.BlockLocators)
	if err != nil {
		// Already a *hcashjson.RPCError
		return nil, err
	}
	...
	return &hcashjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &hcashjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := hcashjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := hcashjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &hcashjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hcashjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*hcashjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &hcashjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch hcashjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case hcashjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case hcashjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case hcashjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case hcashjson.GRMRevocations:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSRtx
		case hcashjson.GRMAll:
			// Nothing to do
		default:
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := hcashutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := hcashutil.Amount(nextSdiff)

	sDiffResult := &hcashjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []hcashjson.VersionCount {
	sorted := make([]hcashjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, hcashjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*hcashjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := hcashjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]hcashjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := hcashjson.VersionInterval{
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.GetStakeVersionsCmd)
	...
	result := hcashjson.GetStakeVersionsResult{
		StakeVersions: make([]hcashjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := hcashjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]hcashjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				hcashjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*hcashjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := hcashjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var txFromMempool *hcashutil.Tx
	if includeMempool {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &hcashjson.RPCError{
				Code: hcashjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &hcashjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         hcashutil.Amount(value).ToUnit(hcashutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: hcashjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkRequest(s *rpcServer) (interface{}, error) {
	...
	// generated.
	lastTxUpdate := s.server.txMemPool.LastUpdated()
	latestHash, latestHeight, _ := s.server.blockManager.chainState.Best()
	latestKeyHeight := s.server.blockManager.chainState.newestKeyHeight
	...
	if msgBlock == nil || state.prevHash == nil ||
		...
		if templateCopy.Height > 1 {
			...
			state.extraNonce++
			ens[0]++
			err := UpdateExtraNonce(msgBlock, uint32(latestHeight+1), uint32(latestKeyHeight), ens)
			if err != nil {
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	// TODO: Modified
	tempBlock := hcashutil.NewBlockDeepCopy(blockInfo.msgBlock)
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return hcashjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return hcashjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
func handleRebroadcastMissed(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	hash, height ,_:= s.server.blockManager.chainState.Best()
	mt, err := s.server.blockManager.chain.MissedTickets()
	if err != nil {
		return nil, rpcInternalError("Could not get missed tickets "+
			err.Error(), "")
...
func handleRebroadcastWinners(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	hash, height, keyHeight:= s.server.blockManager.chainState.Best()
	chain := s.server.blockManager.chain
	var err error
	if blockchain.HashToBig(hash).Cmp(blockchain.CompactToBig(s.server.blockManager.chainState.newestBits)) > 0{
		hash = &(s.server.blockManager.chainState.curPrevKeyHash)
		height, err = chain.BlockHeightByHash(&(s.server.blockManager.chainState.curPrevKeyHash))
		if err != nil{
			return nil, err
		}
		keyHeight, err = chain.KeyHeightByHeight(height, nil)
		if err != nil{
			return nil, err
		}
	}
	...
	*/
	//for i := range blocks {
		winningTickets, _, _, err :=
			s.server.blockManager.chain.LotteryDataForBlock(hash)
		if err != nil {
		...
		if len(winningTickets) > 0 {
			...
			//fmt.Println(ntfnData.BlockHash)
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]hcashjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]hcashjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := hcashutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]hcashjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &hcashjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     hcashutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr hcashutil.Address, numToSkip, numRequested uint32) ([]*hcashutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*hcashjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]hcashjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.SendRawTransactionCmd)
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.SetGenerateCmd)
...
func min(s []hcashutil.Amount) hcashutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []hcashutil.Amount) hcashutil.Amount {
	max := hcashutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []hcashutil.Amount) hcashutil.Amount {
	sum := hcashutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / hcashutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []hcashutil.Amount) hcashutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(hcashutil.AmountSorter(s))
...
func stdDev(s []hcashutil.Amount) hcashutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := hcashutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *hcashjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]hcashutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (hcashutil.Amount(txD.Fee)) * 1000 /
				hcashutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &hcashjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *hcashutil.Tx) hcashutil.Amount {
	var in hcashutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += hcashutil.Amount(txIn.ValueIn)
	}
	var out hcashutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += hcashutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / hcashutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*hcashjson.FeeInfoBlock, error) {
	...
	txFees := make([]hcashutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*hcashjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []hcashutil.Amount
	for i := range hashes {
	...
	return &hcashjson.FeeInfoWindow{
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []hcashjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []hcashjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &hcashjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.TicketsForAddressCmd)
	...
	reply := &hcashjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	end := uint32(bestKeyHeight)
	if c.End != nil {
		end = *c.End
	}
	if start > end {
		return nil, rpcInvalidError("Start keyHeight %v is beyond end "+
			"keyHeight %v", start, end)
	}
	if end > uint32(bestHeight) {
		return nil, rpcInvalidError("End keyHeight %v is beyond "+
			"blockchain tip keyHeight %v", end, bestKeyHeight)
	}
	...
	totalValue := int64(0)
	index := bestHeight
	blockHeader, err := s.chain.HeaderByHeight(int64(index))
	...
	for {
		...
		blockKeyHeight = blockHeader.KeyHeight
		if blockKeyHeight <= uint32(activeNetParams.CoinbaseMaturity) || blockKeyHeight < start {
			break
		}
		//if this block is a keyBlock,then calculate it
		blockHash := blockHeader.BlockHash()
		if blockchain.HashToBig(&blockHash).Cmp(blockchain.CompactToBig(blockHeader.Bits)) <= 0{
	...
	return hcashutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange hcashjson.FeeInfoRange
	...
	feeInfoRange = hcashjson.FeeInfoRange{
	...
	return &hcashjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.ValidateAddressCmd)

	result := hcashjson.ValidateAddressChainResult{}
	addr, err := hcashutil.DecodeAddress(c.Address)
	if err != nil {
...
func verifyChain(s *rpcServer, level, depth int64) error {
	...
	for height := best.Height; height > finishHeight; height-- {
		...
		if level > 0 {
			err := blockchain.CheckBlockSanity(s.chain, block,
				s.server.timeSource,
...
func handleVerifyChain(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.VerifyChainCmd)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*hcashjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*hcashutil.AddressPubKeyHash); !ok {
		return nil, &hcashjson.RPCError{
			Code:    hcashjson.ErrRPCType,
	...
	if wasCompressed {
		serializedPK = hcashPK.SerializeCompressed()
	} else {
		serializedPK = hcashPK.SerializeUncompressed()
	}
	address, err := hcashutil.NewAddressSecpPubKey(serializedPK,
		activeNetParams.Params)
	if err != nil {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, hcashjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *hcashjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(hcashjson.Error); ok &&
			jerr.Code == hcashjson.ErrUnregisteredMethod {

			parsedCmd.err = hcashjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
// the type *hcashjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *hcashjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*hcashjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return hcashjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="hcashd RPC"`)
