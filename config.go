...
import (
	...
	"github.com/HcashOrg/hcashd/connmgr"
	"github.com/HcashOrg/hcashd/database"
	_ "github.com/HcashOrg/hcashd/database/ffldb"
	"github.com/HcashOrg/hcashd/mempool"
	"github.com/HcashOrg/hcashd/sampleconfig"
	"github.com/HcashOrg/hcashutil"
	flags "github.com/jessevdk/go-flags"
)

const (
	defaultConfigFilename        = "hcashd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "hcashd.log"
	defaultMaxPeers              = 125
...
var (
	defaultHomeDir     = hcashutil.AppDataDir("hcashd", false)
...
// config defines the configuration options for hcashd.
//
...
type config struct {
	...
	Listeners            []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 18008, testnet: 18008)"`
	...
	RPCListeners         []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 18009, testnet: 12009)"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in HCASH/kB to be considered a non-zero fee."`
...
// createDefaultConfig copies the file sample-hcashd.conf to the given destination path,
...
//
// The above results in hcashd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	// Update the home directory for hcashd if specified. Since the home
	...
	cfg.minRelayTxFee, err = hcashutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]hcashutil.Address, 0, len(cfg.GetWorkKeys)+
		len(cfg.MiningAddrs))
	for _, strAddr := range cfg.GetWorkKeys {
		addr, err := hcashutil.DecodeAddress(strAddr)
		if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			hcashdLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		hcashdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// hcashdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func hcashdDial(addr net.Addr) (net.Conn, error) {
...
// hcashdLookup returns the correct DNS lookup function to use depending on the
...
func hcashdLookup(host string) ([]net.IP, error) {
