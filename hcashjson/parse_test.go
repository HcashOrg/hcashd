...
func TestEncodeConcatenatedHashes(t *testing.T) {
	// Input Hash slice. Data taken from Hypercash's first three mainnet blocks.
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := hcashjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	// Test data taken from Hypercash's first three mainnet blocks
	...
	decodedHashes, err := hcashjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := hcashjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
	...
	_, err = hcashjson.EncodeConcatenatedVoteBits(testVbs)
	if err == nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		hcashjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	decodedSlice, err = hcashjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
