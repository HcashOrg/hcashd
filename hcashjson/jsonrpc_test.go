...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if hcashjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *hcashjson.RPCError {
				return hcashjson.NewRPCError(hcashjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := hcashjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := hcashjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := hcashjson.Error{Code: hcashjson.ErrInvalidType}
	_, err = hcashjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(hcashjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = hcashjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		in   *hcashjson.RPCError
		want string
	}{
		{
			hcashjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			hcashjson.ErrRPCMethodNotFound,
