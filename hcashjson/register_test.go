...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   hcashjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{hcashjson.UFWalletOnly, "UFWalletOnly"},
		{hcashjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{hcashjson.UFNotification, "UFNotification"},
		{hcashjson.UFWalletOnly | hcashjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{hcashjson.UFWalletOnly | hcashjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := hcashjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrDuplicateMethod},
		},
		{
			...
			flags: hcashjson.TstHighestUsageFlagBit,
			err:   hcashjson.Error{Code: hcashjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrEmbeddedType},
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrUnexportedField},
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrNonOptionalField},
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrNonOptionalDefault},
		},
		{
			...
			err: hcashjson.Error{Code: hcashjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := hcashjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	hcashjson.MustRegisterCmd("panicme", 0, 0)
}
