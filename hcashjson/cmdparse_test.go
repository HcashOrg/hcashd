...
func TestAssignField(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := hcashjson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			method: "getblock",
			args:   []interface{}{},
			err:    hcashjson.Error{Code: hcashjson.ErrNumParams},
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := hcashjson.NewCmd(test.method, test.args...)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := hcashjson.MarshalCmd(test.id, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error type - got `%T` (%v), want `%T`",
				i, test.name, err, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := hcashjson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error type - got `%T` (%v), want `%T`",
				i, test.name, err, err, test.err)
			continue
