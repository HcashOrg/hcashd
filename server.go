...
var (
	...
	// ourselves to other hypercash peers.
	userAgentName = "hcashd"
...
// broadcastMsg provides the ability to house a hypercash message to be broadcast
...
// server provides a hypercash server for handling communications to and from
// hypercash peers.
type server struct {
...
func (sp *serverPeer) newestBlock() (*chainhash.Hash, int64, int64, error) {
	...
	return best.Hash, best.Height, realKeyHeight, nil
}
...
func (sp *serverPeer) OnGetMiningState(p *peer.Peer, msg *wire.MsgGetMiningState) {
	...
	newest, height, keyHeight := bm.chainState.Best()

	// Send out blank mining states if it's early in the blockchain.
	if keyHeight < activeNetParams.StakeValidationHeight-1 {
...
func (s *server) AnnounceNewTransactions(newTxs []*hcashutil.Tx) {
...
func (s *server) handleUpdatePeerHeights(state *peerState, umsg updatePeerHeightsMsg) {
	state.forAllPeers(func(sp *serverPeer) {
		...
		if *latestBlkHash == *umsg.newHash {
			...
			sp.UpdateLastKeyBlockHeight(umsg.newKeyHeight)
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, hcashdLookup, func(addrs []*wire.NetAddress) {
...
// AddedNodeInfo returns an array of hcashjson.GetAddedNodeInfoResult structures
...
func (s *server) UpdatePeerHeights(latestBlkHash *chainhash.Hash, latestHeight int64, latestKeyHeight int64, updateSource *serverPeer) {
	s.peerHeightsUpdate <- updatePeerHeightsMsg{
		newHash:    latestBlkHash,
		newHeight:  latestHeight,
		newKeyHeight: latestKeyHeight,
...
// newServer returns a new hcashd server configured to listen on addr for the
// hypercash network type specified by chainParams.  Use start to begin accepting
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, hcashdLookup)
...
func addrStringToNetAddr(addr string) (net.Addr, error) {
	...
	// The hcashdLookup function will transparently handle performing the
	// lookup over Tor if necessary.
	ips, err := hcashdLookup(host)
	if err != nil {
		return nil, err
	}
	if len(ips) == 0 {
