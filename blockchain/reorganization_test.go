...
func reorgTestShort(t *testing.T) {
	...
	for i := 1; i < forkPoint+1; i++ {
		bl, err := hcashutil.NewBlockFromBytes(blockChain1[int64(i)])
		if err != nil {
			t.Fatalf("NewBlockFromBytes error: %v", err.Error())
		}
	...
	for i := forkPoint; i < finalIdx2; i++ {
		for j := 0; j < 2; j++ {
			bl, err := hcashutil.NewBlockFromBytes(dominant[int64(i+j)])
			if err != nil {
				t.Fatalf("NewBlockFromBytes error: %v", err.Error())
			}
...
func reorgTestForced(t *testing.T) {
	...
	forkPoint := int64(131)
	forkBl, err := hcashutil.NewBlockFromBytes(blockChain[forkPoint])
	if err != nil {
		t.Fatalf("NewBlockFromBytes error: %v", err.Error())
	}
