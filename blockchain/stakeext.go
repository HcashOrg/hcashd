...
func (b *BlockChain) lotteryDataForBlock(hash *chainhash.Hash) ([]chainhash.Hash, int, [6]byte, error) {
	...
	//if !node.isKeyBlock{
	//	winningTickets = make([]chainhash.Hash, 0, 0)
	//}
...
//
// This function is safe for concurrent access.
func (b *BlockChain) TicketsWithAddress(address hcashutil.Address) ([]chainhash.Hash, error) {
	b.chainLock.RLock()
	sn := b.bestNode.stakeNode
	b.chainLock.RUnlock()
...
func (b *BlockChain) TicketPoolValue() (hcashutil.Amount, error) {
	b.chainLock.RLock()
	sn := b.bestNode.stakeNode
	b.chainLock.RUnlock()
	...
	return hcashutil.Amount(amt), nil
}
