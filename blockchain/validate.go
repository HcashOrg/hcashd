...
func IsCoinBase(tx *hcashutil.Tx) bool {
...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// known as an atom.  One hypercash is a quantity of atoms as defined by
	...
	for _, txOut := range tx.TxOut {
		...
		// is detected and reported.  This is impossible for Hypercash,
		...
		if totalAtom > hcashutil.MaxAmount {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfStake(block *hcashutil.Block, posLimit int64) error {
...
func checkProofOfWork(chain *BlockChain, header *wire.BlockHeader, powLimit *big.Int, difficultyRate uint32,
	microBlockValidationHeight uint32, flags BehaviorFlags) error {
	...
	microTarget := big.NewInt(0)
	microTarget.Mul(target, big.NewInt(int64(difficultyRate)))
	...
	if flags&BFNoPoWCheck != BFNoPoWCheck {
		...
		//prevKeyHash := header.PrevKeyBlock
		//prevKeyBlock, err := chain.fetchBlockFromHash(&prevKeyHash)
		//prevKeyBlockHeight := int64(0)
		//if err == nil {
		//	prevKeyBlockHeight = prevKeyBlock.Height()
		//}
		//prevKeyHeight, err := chain.BlockHeightByHash(&prevKeyHash)
		//if err != nil {
		//	prevKeyHeight = 0
		//}
		//heightDiff := header.Height - uint32(prevKeyBlockHeight)

		if header.Height <= microBlockValidationHeight{

			if hashNum.Cmp(target) > 0 {
				str := fmt.Sprintf("the block height  %v, is not larger than " +
					"than microBlockMinHeight %v: ", header.Height, microBlockValidationHeight)
				return ruleError(ErrHighHash, str)
			}
		} else {
			if hashNum.Cmp(microTarget) > 0 {
				str := fmt.Sprintf("block hash of %064x is higher than"+
					" expected max of %064x", hashNum, microTarget)
			return ruleError(ErrHighHash, str)
			}
		}
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(chain *BlockChain, block *hcashutil.Block, powLimit *big.Int, difficultyRate uint32, min uint32) error {
	return checkProofOfWork(chain, &block.MsgBlock().Header, powLimit, difficultyRate, min, BFNone)
}
...
func checkBlockHeaderSanity(chain *BlockChain, block *hcashutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {
	powLimit := chainParams.PowLimit
	posLimit := chainParams.MinimumStakeDiff
	difficultyRate := chainParams.DifficultyRate
	min := chainParams.MicroBlockValidationHeight
	...
	// the bits.
	err := checkProofOfWork(chain, header, powLimit, difficultyRate, uint32(min), flags)
	if err != nil {
		return err
	}
...
func checkBlockSanity(chain *BlockChain, block *hcashutil.Block, timeSource MedianTimeSource, flags BehaviorFlags, chainParams *chaincfg.Params) error {

	msgBlock := block.MsgBlock()
	header := &msgBlock.Header
	err := checkBlockHeaderSanity(chain, block, timeSource, flags, chainParams)
	isKeyBlock := false

	noPowCheck := flags&BFNoPoWCheck == BFNoPoWCheck
	if err != nil {
		return err
	}
	...
	// modified by shanyonglong
	txStart := 1
	if noPowCheck {
		if !IsCoinBaseTx(transactions[1].MsgTx()) {
			return ruleError(ErrFirstTxNotCoinbase, "first transaction in "+
				"block is not a coinbase")
		}
		txStart = 2
		...
		if HashToBig(block.Hash()).Cmp(targetDifficulty) <= 0 {
			//is key block
			if !IsCoinBaseTx(transactions[1].MsgTx()) {
				return ruleError(ErrFirstTxNotCoinbase, "first transaction in "+
					"block is not a coinbase")
	...
	// A block may not have more than one coinbase.
	for i, tx := range transactions[txStart:] {
	...
	totalVotes := 0
	totalRevocations := 0
	//isKeyBlock := HashToBig(block.Hash()).Cmp(CompactToBig(block.MsgBlock().Header.Bits)) <= 0
	...
	if isKeyBlock {
		...
		if int64(block.MsgBlock().Header.KeyHeight + 1) >= chainParams.StakeValidationHeight &&
	...
	merkles := BuildMerkleTreeStore(block.Transactions(), !(isKeyBlock || noPowCheck))
	...
	if isKeyBlock {
		...
		merkleStake := BuildMerkleTreeStore(block.STransactions(), false)
	...
	if int64(header.KeyHeight + 1) < chainParams.StakeValidationHeight {
...
// sane before continuing with block processing.  These checks are context
// free.
func CheckBlockSanity(chain *BlockChain, block *hcashutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
	return checkBlockSanity(chain, block, timeSource, BFNone, chainParams)
}
...
// context free.
func CheckWorklessBlockSanity(chain *BlockChain, block *hcashutil.Block, timeSource MedianTimeSource, chainParams *chaincfg.Params) error {
	return checkBlockSanity(chain , block, timeSource, BFNoPoWCheck, chainParams)
}
...
func (b *BlockChain) checkBlockHeaderContext(header *wire.BlockHeader, prevNode *blockNode, flags BehaviorFlags) error {
	...
	// Ensure the votebits & 0x1 is same as the parent block if the parent block is a microblock

	if header.Height > 1 && (header.VoteBits & hcashutil.BlockValid) != (prevNode.header.VoteBits & hcashutil.BlockValid) {
		if HashToBig(&(prevNode.hash)).Cmp(CompactToBig(prevNode.header.Bits)) > 0 {
			str := fmt.Sprintf("VoteBits of block %v is not same as the parent micro block %v", header.BlockHash(), prevNode.hash)
			return ruleError(ErrVoteBitsNotCompatible, str)
		}
	}
...
//
// Hypercash: Check the stake transactions to make sure they don't have this txid
// too.
func (b *BlockChain) checkDupTxs(txSet []*hcashutil.Tx, view *UtxoViewpoint) error {
...
func (b *BlockChain) CheckBlockStakeSanity(stakeValidationHeight int64, node *blockNode, block *hcashutil.Block, parent *hcashutil.Block, chainParams *chaincfg.Params) error {
	...
	for i, tx := range stakeTransactions {
		...
		if (isSSGen && node.keyHeight < stakeEnabledHeight + 1) ||
			(isSSRtx && node.keyHeight < stakeEnabledHeight + 1) {
			errStr := fmt.Sprintf("block contained SSGen or SSRtx "+
				"transaction at idx %v, which was before stake"+
				" voting was enabled; block key Height %v, stake "+
				"enabled height %v", i, node.keyHeight,
				stakeEnabledHeight)
	...
	if node.keyHeight + 1 < chainParams.StakeValidationHeight &&
	...
	//modify node.height->int64(block.MsgBlock().Header.KeyHeight+1)
	if int64(node.keyHeight+1) < stakeValidationHeight {
	...
	for _, staketx := range stakeTransactions {
		...
		if is, _ := stake.IsSSGen(msgTx); is {
			...
			if hcashutil.IsFlagSet16(ssGenVoteBits, hcashutil.BlockValid) {
				voteYea++
			} else {
				voteNay++
			}
			...
			voteHash, _, voteKeyHeight, err := stake.SSGenBlockVotedOn(msgTx)
			if err != nil {
			...
			if !(voteHash.IsEqual(prevKeyBlockHash)) ||
				(voteKeyHeight != uint32(block.MsgBlock().Header.KeyHeight)-1) {
				txHash := msgTx.TxHash()
				errStr := fmt.Sprintf("Error in stake "+
					"consensus: SSGen %v voted on block "+
					"%v at height %v, however it was "+
					"found inside block %v at height %v!",
					txHash, voteHash, voteKeyHeight,
					prevKeyBlockHash, block.MsgBlock().Header.KeyHeight-1)
			...
			if !(voteHash.IsEqual(prevKeyBlockHash)){
				txHash := msgTx.TxHash()
				errStr := fmt.Sprintf("Error in stake consensus: SSGen %v voted on block "+
					"%v , however it was "+
					"found inside block %v",
					txHash, voteHash, voteHeight,
...
// owner of the hypercash and therefore allowed to spend them.  As it checks the
...
func CheckTransactionInputs(chain *BlockChain, subsidyCache *SubsidyCache, tx *hcashutil.Tx, txHeight int64, txKeyHeight int64, utxoView *UtxoViewpoint,
	...
	if msgTx.Expiry != wire.NoExpiryValue {
		if txKeyHeight >= int64(msgTx.Expiry) {
			errStr := fmt.Sprintf("Transaction indicated an "+
				"expiry of %v while the current height is %v",
				tx.MsgTx().Expiry, txKeyHeight)
			return 0, 0, 0, ruleError(ErrExpiredTx, errStr)
	...
	//var stakeSubsidy int64
	var txAtomIn int64
	var stakeAtomIn int64
	...
	// -------------------------------------------------------------------
	// Hypercash stake transaction testing.
	// -------------------------------------------------------------------
	...
	if isSStx {
		...
		for idx, txIn := range msgTx.TxIn {
			...
			if !exists || utxoEntry == nil {
				str := fmt.Sprintf("unable to find input "+
					...
					txInHash, txHash)
				return 0, 0, 0, ruleError(ErrMissingTx, str)
			}
			...
			if utxoEntry.IsOutputSpent(originTxIndex) {
				...
				return 0, 0, 0, ruleError(ErrDoubleSpend, str)
			}
			...
			if !(class == txscript.PubKeyHashTy ||
				...
				return 0, 0, 0, ruleError(ErrSStxInScrType, errStr)
			}
		...
		if err != nil {
			errStr := fmt.Sprintf("SStx output commitment amounts"+
				...
				err)
			return 0, 0, 0, ruleError(ErrSStxCommitment, errStr)
		}
	}
	...
	if isSSGen {
		...
		// height.
		if txKeyHeight < stakeEnabledHeight + 1 {
			errStr := fmt.Sprintf("SSGen tx appeared in block "+
				"height %v before stake enabled height %v",
				txKeyHeight, stakeEnabledHeight + 1)
			return 0, 0, 0, ruleError(ErrInvalidEarlyStakeTx, errStr)
		...
		_, _, keyHeightVotingOn, err := stake.SSGenBlockVotedOn(msgTx)
		if err != nil {
			errStr := fmt.Sprintf("Could not parse SSGen block "+
				"vote information from SSGen %v:  %v", txHash,
				err)
			return 0, 0, 0, ruleError(ErrUnparseableSSGen, errStr)
		}

		stakeVoteSubsidy := CalcStakeVoteSubsidy(subsidyCache,
			int64(keyHeightVotingOn), chainParams)
		...
		if nullIn.ValueIn != stakeVoteSubsidy {
			...
			return 0, 0, 0, ruleError(ErrBadStakebaseAmountIn, errStr)
		}
		...
		if !exists || utxoEntrySstx == nil {
			errStr := fmt.Sprintf("Unable to find input sstx "+
				...
				txHash)
			return 0, 0, 0, ruleError(ErrMissingTx, errStr)
		}
		...
		if utxoEntrySstx.TransactionType() != stake.TxTypeSStx {
			errStr := fmt.Sprintf("Input transaction %v for SSGen"+
				...
				sstxHash)
			return 0, 0, 0, ruleError(ErrInvalidSSGenInput, errStr)
		}
		...
		if len(minOutsSStx) == 0 {
			return 0, 0, 0, AssertError("missing stake extra data for " +
		...
		if err != nil {
			...
			return 0, 0, 0, ruleError(ErrSSGenPayeeOuts, errStr)
		}
		...
		if (len(sstxPayTypes) != len(ssgenPayTypes)) ||
			...
			return 0, 0, 0, ruleError(ErrSSGenPayeeNum, errStr)
		}
		...
		originKeyHeight,err := chain.KeyHeightByHeight(originHeight, keyHeightCache)
		if err != nil{
			return 0, 0, 0, err
		}
		blocksSincePrev := txKeyHeight - originKeyHeight
		...
		if blocksSincePrev <= ticketMaturity {
			errStr := fmt.Sprintf("tried to spend sstx output "+
				...
				"blocks", sstxHash, originKeyHeight, txKeyHeight,
				ticketMaturity)
			return 0, 0, 0, ruleError(ErrSStxInImmature, errStr)
		}
	}
	...
	if isSSRtx {
		...
		if utxoEntrySstx.TransactionType() != stake.TxTypeSStx {
			...
			return 0, 0, 0, ruleError(ErrInvalidSSRtxInput, errStr)
		}
		...
		if err != nil {
			...
			return 0, 0, 0, ruleError(ErrSSRtxPayees, errStr)
		}
		...
		if (len(sstxPkhs) != len(ssrtxPkhs)) ||
			...
			return 0, 0, 0, ruleError(ErrSSRtxPayeesMismatch, errStr)
		}
		...
		//    and the block in which the SStx was originally found in.
		originKeyHeight, err := chain.KeyHeightByHeight(utxoEntrySstx.BlockHeight(), keyHeightCache)
		if err != nil{
			return 0, 0, 0, err
		}
		blocksSincePrev := txKeyHeight - originKeyHeight
	...
	// -------------------------------------------------------------------
	// Hypercash general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	for idx, txIn := range msgTx.TxIn {
		...
		if checkFraudProofFlag != FraudProofNoCheck {
			if txIn.ValueIn !=
				...
				str := fmt.Sprintf("bad fraud check value in "+
					...
					utxoEntry.AmountByIndex(originTxIndex),
					txIn.ValueIn, idx)
				return 0, 0, 0, ruleError(ErrFraudAmountIn, str)
			}
		...
		originKeyHeight, err := chain.KeyHeightByHeight(utxoEntry.BlockHeight(), keyHeightCache)
		if err != nil{
			return 0, 0, 0, err
		}
		if utxoEntry.IsCoinBase() {
			...
			if blocksSincePrev < coinbaseMaturity {
				str := fmt.Sprintf("tx %v tried to spend "+
					...
					txInHash, originKeyHeight, txKeyHeight,
					coinbaseMaturity)
				return 0, 0, 0, ruleError(ErrImmatureSpend, str)
			}
		}
		...
		if utxoEntry.HasExpiry() {
			...
			if blocksSincePrev < coinbaseMaturity {
				str := fmt.Sprintf("tx %v tried to spend "+
					...
					txHash, txInHash, originKeyHeight,
					txKeyHeight, coinbaseMaturity)
				return 0, 0, 0, ruleError(ErrExpiryTxSpentEarly, str)
		...
		if indicatedTree != originTxOPTree {
			errStr := fmt.Sprintf("tx %v attempted to spend from "+
				...
				originTxOPTree)
			return 0, 0, 0, ruleError(ErrDiscordantTxTree, errStr)
		}
		...
		if !(isSSGen || isSSRtx) {
			if txscript.GetScriptClass(
				...
				errStr := fmt.Sprintf("Tx %v attempted to "+
					...
					errIsSSRtx.Error())
				return 0, 0, 0, ruleError(ErrTxSStxOutSpend, errStr)
			}
		}
		...
		if scriptClass == txscript.StakeGenTy ||
			...
			blocksSincePrev := txKeyHeight - originKeyHeight
			if blocksSincePrev <
				int64(chainParams.CoinbaseMaturity) {
		...
		if scriptClass == txscript.StakeSubChangeTy {
			...
			if blocksSincePrev <
				...
				str := fmt.Sprintf("tried to spend SStx change"+
					...
					"of %v blocks", txInHash, originKeyHeight,
					txKeyHeight, chainParams.SStxChangeMaturity)
				return 0, 0, 0, ruleError(ErrImmatureSpend, str)
		...
		// hypercash is a quantity of atoms as defined by the AtomPerCoin
		// constant.
		...
		if originTxAtom < 0 {
			...
			return 0, 0, 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxAtom > hcashutil.MaxAmount {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				originTxAtom, hcashutil.MaxAmount)
			return 0, 0, 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if !isSSGen && !isSStx && !isSSRtx {
			lastAtomIn := totalAtomIn
			totalAtomIn += originTxAtom
			txAtomIn += originTxAtom

			if totalAtomIn < lastAtomIn ||
				totalAtomIn > hcashutil.MaxAmount {
				str := fmt.Sprintf("total value of all transaction "+
					"inputs is %v which is higher than max "+
					"allowed value of %v", txAtomIn,
					hcashutil.MaxAmount)
				return 0, 0, 0, ruleError(ErrBadTxOutValue, str)
			}
		} else {
			lastAtomIn := totalAtomIn
			stakeAtomIn += originTxAtom
			totalAtomIn += originTxAtom

			if totalAtomIn < lastAtomIn ||
				totalAtomIn > hcashutil.MaxAmount {
				str := fmt.Sprintf("total value of all stake transaction "+
					"inputs is %v which is higher than max "+
					"allowed value of %v", stakeAtomIn,
					hcashutil.MaxAmount)
				return 0, 0, 0, ruleError(ErrBadTxOutValue, str)
			}
		}
	...
	var txAtomOut int64
	var stakeAtomOut int64
	for i, txOut := range tx.MsgTx().TxOut {
		totalAtomOut += txOut.Value
		if !isSStx && !isSSGen && !isSSRtx {
			txAtomOut += txOut.Value
		} else if !isSSGen{
			stakeAtomOut += txOut.Value
		}
		...
		if !isSStx && !isSSGen && !isSSRtx {
			...
			if (scriptClass == txscript.StakeSubmissionTy) ||
				...
				errStr := fmt.Sprintf("Non-stake tx %v "+
					...
					scriptClass, i)
				return 0, 0, 0, ruleError(ErrRegTxSpendStakeOut, errStr)
			}
			...
			if err != nil {
				return 0, 0, 0, ruleError(ErrScriptMalformed,
					err.Error())
			}
			if op {
				errStr := fmt.Sprintf("Non-stake tx %v "+
					...
					"position %v", txHash, i)
				return 0, 0, 0, ruleError(ErrScriptMalformed, errStr)
			}
		}
	}
	...
	if totalAtomIn < totalAtomOut {
		...
		return 0, 0, 0, ruleError(ErrSpendTooHigh, str)
	}
	/*
	if txAtomIn < txAtomOut {
		str := fmt.Sprintf("total value of all normal transaction inputs for "+
			"transaction %v is %v which is less than the amount "+
			"spent of %v", txHash, txAtomIn, txAtomOut)
		return 0, 0, 0, ruleError(ErrSpendTooHigh, str)
	}

	if stakeAtomIn < stakeAtomOut {
		str := fmt.Sprintf("total value of all transaction inputs for "+
			"transaction %v is %v which is less than the amount "+
			"spent of %v", txHash, stakeAtomIn, stakeAtomOut)
		return 0, 0, 0, ruleError(ErrSpendTooHigh, str)
	}
	*/
	...
	txFees := txAtomIn - txAtomOut
	stakeFees := stakeAtomIn - stakeAtomOut

	return txFeeInAtom, txFees, stakeFees, nil
}
...
// txscript.
func CountSigOps(tx *hcashutil.Tx, isCoinBaseTx bool, isSSGen bool) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *hcashutil.Tx, isCoinBaseTx bool, isStakeBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkNumSigOps(tx *hcashutil.Tx, utxoView *UtxoViewpoint, index int, txTree bool, cumulativeSigOps int, isKeyBlock bool, isMining bool) (int, error) {
	msgTx := tx.MsgTx()
	isSSGen, _ := stake.IsSSGen(msgTx)

	isCoinbase := (index == 0) && txTree
	if isMining || isKeyBlock{
		isCoinbase = (index == 0 || index == 1) && txTree
	}

	numsigOps := CountSigOps(tx, isCoinbase, isSSGen)
	...
	numP2SHSigOps, err := CountP2SHSigOps(tx, isCoinbase,
		isSSGen, utxoView)
	if err != nil {
...
func checkStakeBaseAmounts(subsidyCache *SubsidyCache, keyHeight int64, params *chaincfg.Params, txs []*hcashutil.Tx, utxoView *UtxoViewpoint) error {
	for _, tx := range txs {
		msgTx := tx.MsgTx()
		if is, _ := stake.IsSSGen(msgTx); is {
			// Ensure the input is available.
...
func getStakeBaseAmounts(txs []*hcashutil.Tx, utxoView *UtxoViewpoint) (int64, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
...
func getStakeTreeFees(subsidyCache *SubsidyCache, keyHeight int64, params *chaincfg.Params, txs []*hcashutil.Tx, utxoView *UtxoViewpoint) (hcashutil.Amount, error) {
	totalInputs := int64(0)
	totalOutputs := int64(0)
	for _, tx := range txs {
		msgTx := tx.MsgTx()
	...
	return hcashutil.Amount(totalInputs - totalOutputs), nil
}
...
func (b *BlockChain) checkTransactionsAndConnect(subsidyCache *SubsidyCache, inputFees hcashutil.Amount, node *blockNode,
	txs []*hcashutil.Tx, utxoView *UtxoViewpoint, stxos *[]spentTxOut, txTree bool, isMining bool, keyHeightCache map[int64]int64) error {
	...
	// are out of bounds.
	totalFees := int64(0) // Stake tx tree carry forward
	...
	for idx, tx := range txs {
		...
		cumulativeSigOps, err = checkNumSigOps(tx, utxoView, idx,
			txTree, cumulativeSigOps, node.isKeyBlock, isMining)
		if err != nil {
			return err
		}
	...
	if txTree { //TxTreeRegular
		...
		if node.keyHeight == 0 && len(b.chainParams.BlockOneLedger) != 0{
			expAtomOut = subsidyCache.CalcBlockSubsidy(node.keyHeight)
		} else {
			if node.isKeyBlock || isMining{
				for _, txOut := range txs[1].MsgTx().TxOut {
					totalAtomOutRegular += txOut.Value
				}

				subsidyWork := CalcBlockWorkSubsidy(subsidyCache,
					node.keyHeight, node.header.Voters, b.chainParams)
				subsidyTax := CalcBlockTaxSubsidy(subsidyCache,
					node.keyHeight, node.header.Voters, b.chainParams)

				extraFees := int64(0)
				curHash := &node.parent.hash
				for {
					if curHash.IsEqual(zeroHash) {
						break
					}
					if curHash.IsEqual(&node.header.PrevKeyBlock) {
						extraFee, err := b.GetTotalFeesByHash(curHash)
						if err != nil {
							return err
						}
						extraFees += extraFee
		...
		// TODO:maybe contain some mistake
		// Microblocks have no stake
		if !node.isKeyBlock{
			return nil
		}

		if len(txs) == 0 && node.keyHeight+1 < b.chainParams.StakeValidationHeight {
			return nil
		}
		if len(txs) == 0 && node.keyHeight+1 >= b.chainParams.StakeValidationHeight {
		...
		err := checkStakeBaseAmounts(subsidyCache, node.keyHeight,
		...
		if node.keyHeight + 1 >= b.chainParams.StakeValidationHeight {
			...
			expAtomOut = CalcStakeVoteSubsidy(subsidyCache,
				node.keyHeight-1, b.chainParams) *
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *hcashutil.Block, utxoView *UtxoViewpoint,
	stxos *[]spentTxOut, isMining bool, keyHeightCache map[int64]int64) error {
	...
	if isMining || node.isKeyBlock {
		err = CoinbasePaysTax(b.subsidyCache, block.Transactions()[0],
			node.header.KeyHeight, node.header.Voters, b.chainParams)
		if err != nil {
			return err
		}
	}
	...
	regularTxTreeValid := hcashutil.IsFlagSet16(node.header.VoteBits,
		hcashutil.BlockValid)
	//thisNodeStakeViewpoint := ViewpointCurrentStake
	//thisNodeRegularViewpoint := ViewpointCurrentRegular

	if !regularTxTreeValid {
		regularTxTreeValid = HashToBig(parentBlock.Hash()).Cmp(CompactToBig(parentBlock.MsgBlock().Header.Bits)) > 0
	}
	...
	if regularTxTreeValid {
		...
		utxoView.SetStakeViewpoint(ViewpointPrevRegular)
	...
	utxoView.SetStakeViewpoint(ViewpointCurrentStake)
	...
	err = b.checkTransactionsAndConnect(b.subsidyCache, 0, node,
		block.STransactions(), utxoView, stxos, false, isMining, keyHeightCache)
	if err != nil {
	...
	stakeTreeFees, err := getStakeTreeFees(b.subsidyCache, node.keyHeight,
	...
	utxoView.SetStakeViewpoint(ViewpointCurrentRegular)
	...
	err = b.checkTransactionsAndConnect(b.subsidyCache, stakeTreeFees, node,
		block.Transactions(), utxoView, stxos, true, isMining, keyHeightCache)
	if err != nil {
	...
	if node.height > 1 && stxos != nil {
		idx, err := utxoView.disconnectTransactionSlice(block.Transactions(),
			node.height, stxos, isMining || node.isKeyBlock)
		if err != nil {
			return err
		}
		stxosDeref := *stxos
...
func (b *BlockChain) CheckConnectBlock(block *hcashutil.Block, isMining bool) error {
	b.chainLock.Lock()
