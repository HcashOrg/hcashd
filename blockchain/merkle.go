...
// children nodes.  A diagram depicting how this works for hypercash transactions
...
// will be nil.
func BuildMerkleTreeStore(transactions []*hcashutil.Tx, compressed bool) []*chainhash.Hash {
	// If there's an empty stake tree, return totally zeroed out merkle tree root
	// only.
	var ens chainhash.Hash
	offset := 0
	if compressed {
		offset = 1
	}
	...
	nextPoT := nextPowerOfTwo(len(transactions) + offset)
	arraySize := nextPoT*2 - 1
	merkles := make([]*chainhash.Hash, arraySize)

	// Create the base transaction hashes and populate the array with them.
	if compressed{
		//item := getCoinbaseHashByTx(transactions[0].MsgTx())
		tx := transactions[0].MsgTx()
		if len(tx.TxOut) < 2 {
			merkles[0] = &chainhash.Hash{}
			return merkles
		}
		if len(tx.TxOut[0].PkScript) < 36 {
			merkles[0] = &chainhash.Hash{}
			return merkles
		}
