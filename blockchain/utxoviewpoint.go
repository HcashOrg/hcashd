...
const (
	...
	ViewpointPrevRegular = StakeViewpoint(iota)
	// viewpointPrevRegular is the viewpoint before the TxTreeRegular of the last block
	// to validate the TxTreeRegular of last block

	ViewpointCurrentStake
	// viewpointCurrentStake is the viewpoint before the TxTreeStake of the current block
	// to validate the TxTreeStake of current block

	ViewpointCurrentRegular
	// viewpointCurrentStake is the viewpoint before the TxTreeStake of the current block
	// to validate the TxTreeStake of current block
...
func (view *UtxoViewpoint) AddTxOuts(tx *hcashutil.Tx, blockHeight int64,
	blockIndex uint32) {
	msgTx := tx.MsgTx()
...
func (view *UtxoViewpoint) connectTransaction(tx *hcashutil.Tx, blockHeight int64,
...
func (b *BlockChain) connectTransactions(view *UtxoViewpoint, block *hcashutil.Block,
	parent *hcashutil.Block, stxos *[]spentTxOut) error {
	...
	if parent != nil && block.Height() != 0 {
		...
		regularTxTreeValid := hcashutil.IsFlagSet16(votebits, hcashutil.BlockValid)
		if !regularTxTreeValid{
			regularTxTreeValid = HashToBig(parent.Hash()).Cmp(CompactToBig(parent.MsgBlock().Header.Bits)) > 0
		}
	...
	for i, stx := range block.STransactions() {
		view.SetStakeViewpoint(ViewpointCurrentStake)
		err := view.fetchInputUtxos(b.db, block, parent)
		if err != nil {
			return err
		}
...
func (b *BlockChain) disconnectTransactions(view *UtxoViewpoint,
	block *hcashutil.Block, parent *hcashutil.Block, stxos []spentTxOut) error {
	...
	if parent != nil && block.Height() != 0 {
		...
		if regularTxTreeValid {
			...
			for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
				...
				// because the code relies on its existence in the view in order
				// to signal modifications have happened.
				isCoinbase := (txIdx == 0 || (txIdx == 1 && isPrevKeyBlock))
				entry := view.entries[*tx.Hash()]
				if entry == nil {
...
func (view *UtxoViewpoint) disconnectTransactionSlice(transactions []*hcashutil.Tx,
	height int64, stxosPtr *[]spentTxOut, secondCoinbase bool) (int, error) {
	if stxosPtr == nil {
	...
	for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
		...
		// because the code relies on its existence in the view in order
		// to signal modifications have happened.
		isCoinbase := (txIdx == 0) || (txIdx == 1 && secondCoinbase)
		entry := view.entries[*tx.Hash()]
		if entry == nil {
...
func (view *UtxoViewpoint) fetchInputUtxos(db database.DB,
	block, parent *hcashutil.Block) error {
	...
	if viewpoint == ViewpointCurrentStake {
	...
	if viewpoint == ViewpointCurrentRegular {
...
func (b *BlockChain) FetchUtxoView(tx *hcashutil.Tx, treeValid bool) (*UtxoViewpoint,
	error) {
	b.chainLock.RLock()
