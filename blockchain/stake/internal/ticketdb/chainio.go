...
// 1. Remove the n (constant, n=5 for all Hypercash networks) many tickets that were
...
// state in bytes.
var minimumBestChainStateSize = chainhash.HashSize + 4 + 4 + 4 + 8 + 8 + 2
...
func serializeBestChainState(state BestChainState) []byte {
	...
	offset += 4
	dbnamespace.ByteOrder.PutUint32(serializedData[offset:], state.KeyHeight)
	offset += 4
...
func deserializeBestChainState(serializedData []byte) (BestChainState, error) {
	...
	offset += 4
	state.KeyHeight = dbnamespace.ByteOrder.Uint32(serializedData[offset: offset + 4])
	offset += 4
...
func serializeBlockUndoData(utds []UndoTicketData) []byte {
	...
	for _, utd := range utds {
		...
		offset += 4
		dbnamespace.ByteOrder.PutUint32(b[offset:offset+4], utd.TicketKeyHeight)
		offset += 4
...
func deserializeBlockUndoData(b []byte) ([]UndoTicketData, error) {
	...
	for i := 0; i < entries; i++ {
		...
		keyHeight := dbnamespace.ByteOrder.Uint32(b[offset : offset+4])
		offset += 4
...
func DbPutTicket(dbTx database.Tx, ticketBucket []byte, hash *chainhash.Hash,
	height uint32, keyHeight uint32, missed, revoked, spent, expired bool) error {
	meta := dbTx.Metadata()
	bucket := meta.Bucket(ticketBucket)
	k := hash[:]
	v := make([]byte, 9)
	dbnamespace.ByteOrder.PutUint32(v, height)
	dbnamespace.ByteOrder.PutUint32(v[4:], keyHeight)
	v[8] = undoBitFlagsToByte(missed, revoked, spent, expired)
...
func DbLoadAllTickets(dbTx database.Tx, ticketBucket []byte) (*tickettreap.Immutable, error) {
	...
	err := bucket.ForEach(func(k []byte, v []byte) error {
		...
		missed, revoked, spent, expired := undoBitFlagsFromByte(v[8])
		treapValue := &tickettreap.Value{
			Height:  dbnamespace.ByteOrder.Uint32(v[0:4]),
			KeyHeight: dbnamespace.ByteOrder.Uint32(v[4:8]),
			Missed:  missed,
			Revoked: revoked,
			Spent:   spent,
			Expired: expired,
