...
const (
	...
	SSGenBlockReferenceOutSize = 38 + 4
...
var (
	...
	validSSGenReferenceOutPrefix = []byte{0x6a, 0x28}
...
func AddrFromSStxPkScrCommitment(pkScript []byte,
	params *chaincfg.Params) (hcashutil.Address, error) {
	...
	if isP2SH {
		addr, err = hcashutil.NewAddressScriptHashFromHash(hashBytes, params)
	} else {
		addr, err = hcashutil.NewAddressPubKeyHash(hashBytes, params,
...
// ticket commitment pkScript.
func AmountFromSStxPkScrCommitment(pkScript []byte) (hcashutil.Amount, error) {
	if len(pkScript) < SStxPKHMinOutSize {
	...
	return hcashutil.Amount(binary.LittleEndian.Uint64(amtEncoded)), nil
}
...
// has passed IsSSGen.
func SSGenBlockVotedOn(tx *wire.MsgTx) (chainhash.Hash, uint32, uint32, error) {
	// Get the block header hash.
	blockHash, err := chainhash.NewHash(tx.TxOut[0].PkScript[2:34])
	if err != nil {
		return chainhash.Hash{}, 0, 0, err
	}

	// Get the block height.
	height := binary.LittleEndian.Uint32(tx.TxOut[0].PkScript[34:38])
	keyHeight := binary.LittleEndian.Uint32(tx.TxOut[0].PkScript[38:42])

	return *blockHash, height, keyHeight, nil
}
...
// accordingly.
func SetTxTree(tx *hcashutil.Tx) {
