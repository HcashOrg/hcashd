...
import (
	...
	"github.com/HcashOrg/hcashd/blockchain/stake/internal/dbnamespace"
	"github.com/HcashOrg/hcashd/blockchain/stake/internal/ticketdb"
...
func LoadBestNode(dbTx database.Tx, height uint32, keyHeight uint32, blockHash chainhash.Hash, header wire.BlockHeader, params *chaincfg.Params) (*Node, error) {
	...
	realKeyHeight := header.KeyHeight
	if HashToBig(&blockHash).Cmp(CompactToBig(header.Bits)) <= 0 {
		realKeyHeight++
	}
	if realKeyHeight >= uint32(node.params.StakeValidationHeight-1) {
...
// data.
func connectNode(node *Node, header wire.BlockHeader, ticketsSpentInBlock, revokedTickets, newTickets []chainhash.Hash,
	isKeyBlock bool) (*Node, error) {
	if node == nil {
	...
	// StakeEnabledHeight.
	var err error
	if header.KeyHeight >= uint32(connectedNode.params.StakeEnabledHeight) {
		// Basic sanity check.
		...
		if isKeyBlock {
			toExpireKeyHeight := uint32(0)
			if connectedNode.keyHeight > connectedNode.params.TicketExpiry {
				toExpireKeyHeight = connectedNode.keyHeight -
					connectedNode.params.TicketExpiry
			}
			expired := fetchExpired(toExpireKeyHeight, connectedNode.liveTickets)
	...
	for _, newTicket := range newTickets {
		...
		v := &tickettreap.Value{
			...
			KeyHeight: connectedNode.keyHeight,
			Missed:  false,
			Revoked: false,
			Spent:   false,
			Expired: false,
	...
	if realKeyHeight >= uint32(connectedNode.params.StakeValidationHeight-1) {
		...
		if isKeyBlock {
			...
			for i := 0; i < len(nextWinnersKeys); i++ {
				ticketHash := node.nextWinners[i]
				connectedNode.nextWinners = append(connectedNode.nextWinners,
					ticketHash)
				stateBuffer = append(stateBuffer, ticketHash[:]...)
			}
...
func (sn *Node) ConnectNode(header wire.BlockHeader, ticketsSpentInBlock, revokedTickets, newTickets []chainhash.Hash,
	isKeyBlock bool) (*Node, error) {
	return connectNode(sn, header, ticketsSpentInBlock,
		revokedTickets, newTickets, isKeyBlock)
}
...
func disconnectNode(node *Node, parentHeader wire.BlockHeader, parentUtds UndoTicketDataSlice, parentTickets []chainhash.Hash,
	isKeyBlock bool, dbTx database.Tx) (*Node, error) {
	...
	realKeyHeight := parentHeader.KeyHeight
	if isKeyBlock {
		realKeyHeight++
	}
	...
	restoredNode := &Node{
		height:               node.height - 1,
		keyHeight:            parentHeader.KeyHeight,
		liveTickets:          node.liveTickets,
		missedTickets:        node.missedTickets,
		revokedTickets:       node.revokedTickets,
	...
	for _, undo := range node.databaseUndoUpdate {
		...
		v := &tickettreap.Value{
			...
			KeyHeight: undo.TicketKeyHeight,
			Missed:  undo.Missed,
	...
	if !isKeyBlock {
		restoredNode.nextWinners = make([]chainhash.Hash, 0, len(node.nextWinners))
		for i := 0; i < len(node.nextWinners); i++{
			restoredNode.nextWinners = append(restoredNode.nextWinners, node.nextWinners[i])
		}
	}
	for _, ticketHash := range restoredNode.nextWinners{
		stateBuffer = append(stateBuffer, ticketHash[:]...)
	}
	if node.keyHeight + 1 >= uint32(node.params.StakeValidationHeight) {
...
func (sn *Node) DisconnectNode(parentHeader wire.BlockHeader, parentUtds UndoTicketDataSlice, parentTickets []chainhash.Hash,
	isKeyBlock bool, dbTx database.Tx) (*Node, error) {
	return disconnectNode(sn, parentHeader, parentUtds, parentTickets, isKeyBlock, dbTx)
}
	...
	for _, undo := range node.databaseUndoUpdate {
		...
		case !undo.Missed && !undo.Revoked && !undo.Spent:
			err = ticketdb.DbPutTicket(dbTx, dbnamespace.LiveTicketsBucketName,
				&undo.TicketHash, undo.TicketHeight, undo.TicketKeyHeight, undo.Missed, undo.Revoked,
		...
		case undo.Missed && !undo.Revoked:
			...
			err = ticketdb.DbPutTicket(dbTx, dbnamespace.MissedTicketsBucketName,
				&undo.TicketHash, undo.TicketHeight, undo.TicketKeyHeight, true, undo.Revoked,
				undo.Spent, undo.Expired)
			if err != nil {
				return err
			}
...
func WriteDisconnectedBestNode(dbTx database.Tx, node *Node, hash chainhash.Hash, childUndoData UndoTicketDataSlice) error {
	...
	for _, undo := range childUndoData {
		...
		case undo.Missed && undo.Revoked:
			...
			err = ticketdb.DbPutTicket(dbTx, dbnamespace.MissedTicketsBucketName,
				&undo.TicketHash, undo.TicketHeight, undo.TicketKeyHeight, undo.Missed, false,
				undo.Spent, undo.Expired)
			if err != nil {
				return err
			}
		...
		case undo.Missed && !undo.Revoked:
			...
			err = ticketdb.DbPutTicket(dbTx, dbnamespace.LiveTicketsBucketName,
				&undo.TicketHash, undo.TicketHeight, undo.TicketKeyHeight, false, undo.Revoked,
				undo.Spent, false)
			if err != nil {
				return err
			}
...
//
// This compact form is only used in hypercash to encode unsigned 256-bit numbers
