...
func (s *SpendableOut) Amount() hcashutil.Amount {
	return s.amount
}
...
func MakeSpendableOutForTx(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      hcashutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
type Generator struct {
	...
	p2shOpTrueAddr   hcashutil.Address
...
func MakeGenerator(params *chaincfg.Params) (Generator, error) {
	...
	p2shOpTrueAddr, err := hcashutil.NewAddressScriptHash(opTrueScript, params)
	if err != nil {
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) hcashutil.Amount {
	...
	return hcashutil.Amount(subsidy)
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy hcashutil.Amount, blockHeight uint32, numVotes uint16) hcashutil.Amount {
	powProportion := hcashutil.Amount(g.params.WorkRewardProportion)
	totalProportions := hcashutil.Amount(g.params.TotalSubsidyProportions())
	...
	ticketsPerBlock := hcashutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * hcashutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) hcashutil.Amount {
	...
	posProportion := hcashutil.Amount(g.params.StakeRewardProportion)
	totalProportions := hcashutil.Amount(g.params.TotalSubsidyProportions())
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy hcashutil.Amount, blockHeight uint32, numVotes uint16) hcashutil.Amount {
	devProportion := hcashutil.Amount(g.params.BlockTaxProportion)
	totalProportions := hcashutil.Amount(g.params.TotalSubsidyProportions())
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := hcashutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * hcashutil.Amount(numVotes)) / ticketsPerBlock
}
...
func purchaseCommitmentScript(addr hcashutil.Address, amount, voteFeeLimit, revocationFeeLimit hcashutil.Amount) []byte {
...
func (g *Generator) createTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee hcashutil.Amount) *wire.MsgTx {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*hcashutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, hcashutil.NewTx(tx))
	}
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee hcashutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee hcashutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := hcashutil.Amount(g.calcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = hcashutil.Amount(2)
	...
	var stakeTreeFees hcashutil.Amount
	for txIdx, tx := range stakeTxns {
		...
		var inputSum, outputSum hcashutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += hcashutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += hcashutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreatePremineBlock(blockName string, additionalAmount hcashutil.Amount) *wire.MsgBlock {
