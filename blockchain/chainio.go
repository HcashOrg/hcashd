...
import (
	...
	"github.com/HcashOrg/hcashd/blockchain/internal/dbnamespace"
...
func serializeSizeForMinimalOutputs(tx *hcashutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *hcashutil.Tx) int {
...
// The amount is never encoded into spent transaction outputs in Hypercash
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64,
	...
	// since in Hypercash we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *hcashutil.Block,
	parent *hcashutil.Block) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := hcashutil.NewBlock(b.chainParams.GenesisBlock)
...
func (b *BlockChain) initChainState() error {
	...
	err := b.db.View(func(dbTx database.Tx) error {
		...
		blk := hcashutil.NewBlock(&block)
//		header := &block.Header
		node := newBlockNode(blk, ticketsSpentInBlock(blk),
		...
		// set.
		if dbInfo.version >= 2 {
			node.stakeNode, err = stake.LoadBestNode(dbTx, uint32(node.height),uint32(node.keyHeight),
...
func (b *BlockChain) KeyHeightByHeight(height int64, keyHeightCache map[int64]int64) (int64, error){
	...
	err := b.db.View(func(dbTx database.Tx) error {
		var errLocal error
		header, errLocal = dbFetchHeaderByHeight(dbTx, int64(height))
		return errLocal
	})
	if err != nil {
		return int64(0), err
	}
	return int64(header.KeyHeight), nil
}
...
// from the index, and return a hcashutil.Block with the height set.
func dbFetchBlockByHash(dbTx database.Tx, hash *chainhash.Hash) (*hcashutil.Block, error) {
...
// raw block for the provided height, deserialize it, and return a hcashutil.Block
// with the height set.
func dbFetchBlockByHeight(dbTx database.Tx, height int64) (*hcashutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(blockHeight int64) (*hcashutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*hcashutil.Block, error) {
	b.chainLock.RLock()
