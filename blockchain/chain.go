...
type blockNode struct {
	...
	// isKeyBlock indicates whether the block is a key block.
	isKeyBlock bool
}
...
func newBlockNode(block *hcashutil.Block, ticketsSpent []chainhash.Hash, ticketsRevoked []chainhash.Hash, votes []VoteVersionTuple) *blockNode {
	...
	for index, tx := range block.Transactions() {
		...
		totalFees += totalTxIn - totalTxOut
	}
...
type BestState struct {
	...
	KeyHeight    int64           // The keyHeight of the block.
...
// BlockChain provides functions for working with the Hypercash block chain.
...
// StakeVersions is a condensed form of a hcashutil.Block that is used to prevent
// using gigabytes of memory.
// TODO: we will modify KeyHeight for State in the furture.
...
// GetBlockKeyHeight returns the KeyHeight of a block with height
...
func (b *BlockChain) HaveBlockV2(hash *chainhash.Hash) (bool, error) {
	b.chainLock.RLock()
	exists, err := b.blockExistsV2(hash)
	b.chainLock.RUnlock()
...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *hcashutil.Block) {
...
func (b *BlockChain) getKeyGeneration(h chainhash.Hash) ([]chainhash.Hash, error) {
	node, err := b.findNode(&h, maxSearchDepth)
	...
	// Get the parent of this node.
	p, err := b.getPrevKeyNodeFromNode(node)
	if err != nil {
		return nil, fmt.Errorf("block is orphan (parent missing)")
	}
	if p == nil {
	...
	// Store all the hashes in a new slice and return them.
	var allKeyChildren []chainhash.Hash
	processNodes := list.New()
	children := p.children
	for _, child := range children {
		processNodes.PushBack(child)
	}
	//allKeyChildren = append(allKeyChildren, node.hash)
...
func (b *BlockChain) getMatchedDescendants(h chainhash.Hash, v uint16) ([]chainhash.Hash, error) {
	...
	var matchedDescendants []*blockNode
	processNodes := list.New()
	...
	matchedDescendants = append(matchedDescendants, node)
	for processNodes.Len() > 0 {
		firstElement := processNodes.Remove(processNodes.Front())
		processNode := firstElement.(*blockNode)
	...
	sort.Sort(sort.Reverse(byHeight(matchedDescendants)))
	sortedDescendants := make([]chainhash.Hash, 0, len(matchedDescendants))
	for _, desc := range matchedDescendants {
...
func (b *BlockChain) GetKeyGeneration(hash chainhash.Hash) ([]chainhash.Hash, error) {
	return b.getKeyGeneration(hash)
}
...
func (b *BlockChain) loadBlockNode(dbTx database.Tx, hash *chainhash.Hash) (*blockNode, error) {
	...
	blockHeader := block.MsgBlock().Header
	node := newBlockNode(block, ticketsSpentInBlock(block),
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) getPrevNodeFromBlock(block *hcashutil.Block) (*blockNode,
	error) {
	// Genesis block.
	prevHash := &block.MsgBlock().Header.PrevBlock
...
func (b *BlockChain) getPrevKeyNodeFromNode(node *blockNode) (*blockNode, error) {
	...
	if prevKeyNode, ok = b.index[node.header.PrevKeyBlock]; !ok {
		prevKeyNode = node
		for prevKeyNode != nil {
			prevKeyNode, err = b.getPrevNodeFromNode(prevKeyNode)
			if err != nil {
				log.Errorf("getPrevNodeFromNode: %v", err)
				return nil, err
			}
			if prevKeyNode.isKeyBlock {
				break
			}
		}
	}
...
/*
func (b *BlockChain) getPrevKeyNodeFromNode(node *blockNode) (*blockNode, error) {	//tsy
	// Return the existing previous key block node if it's already there.
	if node.parent != nil {
		return node.parent, nil
	}
	...
	// Dynamically load the previous block from the block database, create
	// a new block node for it, and update the memory chain accordingly.
	var prevKeyBlockNode *blockNode
	err := b.db.View(func(dbTx database.Tx) error {
		var err error
		prevKeyBlockNode, err = b.loadBlockNode(dbTx, &node.header.PrevKeyBlock)
		return err
	})
...
func (b *BlockChain) fetchBlockFromHash(hash *chainhash.Hash) (*hcashutil.Block,
	error) {
	...
	var blockMainchain *hcashutil.Block
...
func (b *BlockChain) GetTopBlock() (*hcashutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BestPrevHash() (chainhash.Hash, chainhash.Hash) {
	b.chainLock.Lock()
	defer b.chainLock.Unlock()

	return b.bestNode.header.PrevBlock, b.bestNode.header.PrevKeyBlock
}
...
func (b *BlockChain) getForkKeyNodes(node *blockNode) (*list.List, *list.List, error) {
	// Nothing to detach or attach if there is no node.
	attachNodes := list.New()
	detachNodes := list.New()
	if node == nil {
...
func (b *BlockChain) pushMainChainBlockCache(block *hcashutil.Block) {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *hcashutil.Block) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *hcashutil.Block, view *UtxoViewpoint, stxos []spentTxOut) error {
...
func (b *BlockChain) dropMainChainBlockCache(block *hcashutil.Block) {
	curHash := block.Hash()
	b.mainchainBlockCacheLock.Lock()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *hcashutil.Block, view *UtxoViewpoint) error {
...
// adding the block.
func countNumberOfTransactions(block, parent *hcashutil.Block) uint64 {
	var numTxns uint64
...
func countSpentOutputs(block *hcashutil.Block, parent *hcashutil.Block) int {
	...
	if regularTxTreeValid {
		if len(parent.Transactions()) > startTx {
			for _, tx := range parent.Transactions()[startTx:] {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List,
	...
	detachBlocks := make([]*hcashutil.Block, 0, detachNodes.Len())
	...
	for e := detachNodes.Front(); e != nil; e = e.Next() {
		n := e.Value.(*blockNode)
		var block *hcashutil.Block
		var parent *hcashutil.Block
		var err error
	...
	for e := attachNodes.Front(); e != nil; e = e.Next() {
		...
		// not needed.
		err := b.checkConnectBlock(n, block, view, nil, false, keyHeightCache)
		if err != nil {
			return err
		}
		topBlock = n
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *hcashutil.Block, flags BehaviorFlags) (bool, error) {
	...
	if blockPrevHash == b.bestNode.hash {
		...
		txTreeRegularValid := hcashutil.IsFlagSet16(node.header.VoteBits,
			hcashutil.BlockValid)
...
type IndexManager interface {
	...
	// main chain.
	ConnectBlock(database.Tx, *hcashutil.Block, *hcashutil.Block, *UtxoViewpoint) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain.
	DisconnectBlock(database.Tx, *hcashutil.Block, *hcashutil.Block, *UtxoViewpoint) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	b := BlockChain{
		...
		blockCache:                    make(map[chainhash.Hash]*hcashutil.Block),
		mainchainBlockCache:           make(map[chainhash.Hash]*hcashutil.Block),
	...
	b.subsidyCache = NewSubsidyCache(b.bestNode.keyHeight, b.chainParams)
