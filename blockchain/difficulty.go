...
// CalcWork calculates a work value from difficulty bits.  Hypercash increases
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcNextRequiredDifficulty(curNode *blockNode,					//tsy
	...
	// just return this.
	oldDiff := newCurNode.header.Bits
	oldDiffBig := CompactToBig(newCurNode.header.Bits)

	// We're not at a retarget point, return the oldDiff.
	if (newCurNode.keyHeight+1)%b.chainParams.WorkDiffWindowSize != 0 {
		...
		if b.chainParams.ReduceMinDifficulty {
			...
			allowMinTime := newCurNode.header.Timestamp.Add(reductionTime)

			// For every extra target timespan that passes, we halve the
			// difficulty.
			if newBlockTime.After(allowMinTime) {
				timePassed := newBlockTime.Sub(newCurNode.header.Timestamp)
				...
				oldTarget := CompactToBig(newCurNode.header.Bits)
			...
			prevBits, err := b.findPrevTestNetDifficulty(newCurNode)
			if err != nil {
				return 0, err
			}
			return prevBits, nil
	...
	nextDiffBigMin := CompactToBig(newCurNode.header.Bits)
	nextDiffBigMin.Div(nextDiffBigMin, RAFBig)
	nextDiffBigMax := CompactToBig(newCurNode.header.Bits)
	...
	//oldNode, err = b.getPrevKeyNodeFromNode(oldNode)
	oldNode := newCurNode
	//oldNode := curNode
	windowPeriod := int64(0)
	weights := uint64(0)
	recentTime := newCurNode.header.Timestamp.UnixNano()
	olderTime := int64(0)
...
// weighted averages.
//
// NOTE: This is the original stake difficulty algorithm that was used at Hypercash
// launch.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcNextRequiredStakeDifficultyV1(curNode *blockNode) (int64, error) {
	...
	// Hypercash parameters, but might do weird things if you use custom
	// parameters.
	if curNode == nil ||
...
func (b *BlockChain) calcNextRequiredStakeDifficultyV2(curNode *blockNode) (int64, error) {
	...
	nextHeight := int64(0)
	if curNode != nil {
		nextHeight = curNode.keyHeight + 2
	}
	...
	if nextHeight%intervalSize != 0 || !curNode.isKeyBlock{
		return curDiff, nil
	}
	...
	prevRetargetNode, err := b.ancestorKeyNode(curNode, prevRetargetHeight)
	if err != nil {
		return 0, err
	}
...
func (b *BlockChain) estimateNextStakeDifficultyV2(curNode *blockNode, newTickets int64, useMaxTickets bool) (int64, error) {
	...
	curKeyHeight := int64(0)
	if !curNode.isKeyBlock{
		return curNode.header.SBits,nil
	}
	if curNode != nil {
		//curHeight = curNode.height
		curKeyHeight = curNode.keyHeight + 1
	}
	intervalSize := b.chainParams.StakeDiffWindowSize
	blocksUntilRetarget := intervalSize - curKeyHeight%intervalSize
	nextRetargetHeight := curKeyHeight + blocksUntilRetarget
	...
	remainingImmatureTickets, err := b.sumPurchasedTickets(curNode,
		curKeyHeight-nextMaturityFloor)
	if err != nil {
		return 0, err
	}
	...
	nextMaturityFloorNode, err := b.ancestorKeyNode(curNode, nextMaturityFloor-1)
	if err != nil {
		return 0, err
	}
	curMaturityFloor := curKeyHeight - ticketMaturity
	...
	if nextRetargetHeight > stakeValidationHeight {
		...
		if curKeyHeight < stakeValidationHeight {
