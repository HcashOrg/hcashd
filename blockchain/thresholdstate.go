...
func (b *BlockChain) thresholdState(version uint32, prevNode *blockNode, checker thresholdConditionChecker, cache *thresholdStateCache) (ThresholdStateTuple, error) {
	...
	wantHeight := calcWantHeight(svh,
		...
		prevNode.keyHeight+2)
	var err error
	prevNode, err = b.ancestorKeyNode(prevNode, wantHeight)
	if err != nil {
	...
	for prevNode != nil {
		...
		// confirmation window.
		prevNode, err = b.ancestorKeyNode(prevNode, prevNode.keyHeight-
	...
	for neededNum := len(neededStates) - 1; neededNum >= 0; neededNum-- {
		...
		case ThresholdStarted:
			...
			for i := int64(0); i < confirmationWindow; i++ {
				...
				countNode, err = b.getPrevKeyNodeFromNode(countNode)
				if err != nil {
					return newThresholdState(
						ThresholdFailed, invalidChoice), err
				}
...
func (b *BlockChain) getVoteCounts(node *blockNode, version uint32, d chaincfg.ConsensusDeployment) (VoteCounts, error) {
	...
	keyHeight := calcWantHeight(b.chainParams.StakeValidationHeight,
		int64(b.chainParams.RuleChangeActivationInterval), node.keyHeight+1)

	result := VoteCounts{
		VoteChoices: make([]uint32, len(d.Vote.Choices)),
	}
	countNode := node
	for countNode.keyHeight > keyHeight {
