...
func (b *BlockChain) blockExistsV2(hash *chainhash.Hash) (bool, error) {
	// Check memory chain first (could be main chain or side chain blocks).
	if _, ok := b.index[*hash]; ok {
		return true, nil
	}
	...
	// Check in the database.
	return b.MainChainHasBlock(hash)
}

// keyHashExists determines whether a preKeyHash with the given hash exists either in
// the main chain or any side chains.
//
// This function MUST be called with the chain state lock held (for reads).
...
//TODO : not check preKeyHash in this function ( prevKeyHash will be check in connectBestChain in chain.go)
...
func (b *BlockChain) ProcessBlock(block *hcashutil.Block, flags BehaviorFlags) (bool, bool, error) {
	b.chainLock.Lock()
	...
	exists, err := b.blockExistsV2(blockHash)
	if err != nil {
