...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error
}
...
// be considered valid.
func approvesParent(block *hcashutil.Block) bool {
	return hcashutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
