...
import (
	...
	"github.com/HcashOrg/hcashd/blockchain/internal/progresslog"
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error {
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error {
...
func (m *Manager) Init(chain *blockchain.BlockChain) error {
	...
	var cachedBlock *hcashutil.Block
	...
	var cachedParent *hcashutil.Block
	for height := lowestHeight + 1; height <= bestHeight; height++ {
		var block, parent *hcashutil.Block
		err = m.db.Update(func(dbTx database.Tx) error {
...
func makeUtxoView(dbTx database.Tx, block, parent *hcashutil.Block) (*blockchain.UtxoViewpoint, error) {
	...
	for txIdx, tx := range parentRegularTxs {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			view.AddTxOuts(hcashutil.NewTx(originTx),
	...
	for _, tx := range block.STransactions() {
		...
		for i, txIn := range msgTx.TxIn {
			...
			view.AddTxOuts(hcashutil.NewTx(originTx), int64(wire.NullBlockHeight),
				wire.NullBlockIndex)
		}
	}
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) ConnectBlock(dbTx database.Tx, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error {
...
//
// This is part of the blockchain.IndexManager interface.
func (m *Manager) DisconnectBlock(dbTx database.Tx, block, parent *hcashutil.Block, view *blockchain.UtxoViewpoint) error {
	// Call each of the currently active optional indexes with the block
