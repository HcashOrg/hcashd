...
func TestSchnorrThresholdSigOnBadPk(t *testing.T) {
	...
	for i := 0; i < NUM_TEST; i++ {
		...
		pkBad := BigIntToEncodedBytes(schnorrKeyVec.pkVec[randItem].GetX())
		pos := tRand.Intn(31)
		bitPos := tRand.Intn(7)
		pkBad[pos] ^= 1 << uint8(bitPos)
		schnorrKeyVec.pkVec[randItem].GetX().SetBytes(pkBad[:])

		// recalculate the sum of public key vector
		schnorrKeyVec.pkVecSum = CombinePubkeys(curve, schnorrKeyVec.pkVec)
...
func TestSchnorrThresholdSigOnBadPubNonce(t *testing.T) {
	...
	for i := 0; i < NUM_TEST; i++ {
		...
		pubNonce := BigIntToEncodedBytes(schnorrKeyVec.pubNonceVec[randItem].GetX())
		pos := tRand.Intn(31)
		bitPos := tRand.Intn(7)
		pubNonce[pos] ^= 1 << uint8(bitPos)
		schnorrKeyVec.pubNonceVec[randItem].GetX().SetBytes(pubNonce[:])
