...
// ConversionVector wraps a pointer to a byte array of length 32
type ConversionVector struct {
	bIn *[32]byte
}

// ScalarMultHexVec serves as container for records of
//	multiplication between curve points and scalar in
//	form of hex strings
type ScalarMultHexVec struct {
	bIn  string // Point
	s    string // 32 byte scalar
	bRes string // Resulting point
}

// ScalarMultVec serves as container for records of
//	multiplication between curve points and scalar
type ScalarMultVec struct {
	bIn  *[32]byte // Point
	s    *[32]byte // 32 byte scalar
	bRes *[32]byte // Resulting point
}

// SignatureVerParams serves as container for records of signatures
...
// SchnorrKeyVec wraps key stuff for Schnorr threshold signing
type SchnorrKeyVec struct {
	skVec       []*PrivateKey // secret key vector
	pkVec       []*PublicKey  // public key vector
	secNonceVec []*PrivateKey // secret nonce vector
	pubNonceVec []*PublicKey  // public nonce vector
...
// mockUpPointConversionVectors mocks up some fake conversion vectors for
// testing conversion between curve points and encoded bytes
// @param numCvs	size of conversion vectors to make
func mockUpPointConversionVectors(numCvs int) []ConversionVector {
	const SEED = 54321
	r := rand.New(rand.NewSource(SEED))
	...
	for i := 0; i < numCvs; i++ {
		...
		// !!! Note: no zeroing out the LSB as these aren't points.
		cvs[i] = ConversionVector{bIn}
		r.Seed(int64(i) + SEED)
	}
...
// mockUpScalarMultHexVec fakes a vector of scalar multiplication records as hex strings
func mockUpScalarMultHexVec() []ScalarMultHexVec {
	return []ScalarMultHexVec{
...
// mockUpScalarMultVec fakes a vector of records for multiplication
//	between curve points and scalars
func mockUpScalarMultVec() []ScalarMultVec {
	tvsmh := mockUpScalarMultHexVec()
	var tvsms []ScalarMultVec
	for _, v := range tvsmh {
		bIn, _ := hex.DecodeString(v.bIn)
		s, _ := hex.DecodeString(v.s)
		bRes, _ := hex.DecodeString(v.bRes)
		lv := ScalarMultVec{copyBytes(bIn), copyBytes(s), copyBytes(bRes)}
...
// mockUpSchnorrKeyVec fake a SchnorrKeyVec for test
func mockUpSchnorrKeyVec(curve *TwistedEdwardsCurve, sz int, msg []byte) *SchnorrKeyVec {
	keyVec := new(SchnorrKeyVec)

	keyVec.skVec = mockUpSecKeysByScalars(curve, sz)
	keyVec.pkVec = make([]*PublicKey, sz, sz)
	keyVec.secNonceVec = make([]*PrivateKey, sz, sz)
	keyVec.pubNonceVec = make([]*PublicKey, sz, sz)

	// copy the secret key vector
	// and derive the public key vector
	// and derive the secret nonce vector
	// and derive the public nonce vector
	for j, sk := range keyVec.skVec {
		_, keyVec.pkVec[j], _ = PrivKeyFromScalar(curve, sk.Serialize())

		nonce := nonceRFC6979(curve, keyVec.skVec[j].Serialize(), msg, nil,
		...
		secNonce, pubNonce, err := PrivKeyFromScalar(curve, nonce[:])
		if err != nil {
			//t.Fatalf("unexpected error %s, ", err)
			errStr := "unexpected error" + err.Error()
			panic(errStr)
		}
		if secNonce == nil {
			panic("private nonce shouldn't be nil")
		}
		if pubNonce == nil {
			panic("public nonce shouldn't be nil")
		}

		keyVec.secNonceVec[j], keyVec.pubNonceVec[j] = secNonce, pubNonce
	}

	keyVec.pkVecSum = CombinePubkeys(curve, keyVec.pkVec)
	keyVec.pubNonceVecSum = CombinePubkeys(curve, keyVec.pubNonceVec)
	if nil == keyVec.pubNonceVecSum {
		panic("unexpected sum of public nonces")
	}
...
func mockUpSecKeysByBytes(curve *TwistedEdwardsCurve, i int) []*PrivateKey {
...
// mockUpSecKeysByScalars generates randomly a list of private keys from scalars for
// the given curve
func mockUpSecKeysByScalars(curve *TwistedEdwardsCurve, i int) []*PrivateKey {
	r := rand.New(rand.NewSource(54321))
	...
	for j := 0; j < i; j++ {
		for {
			...
			// mask out the 5 most significant bits
			bIn[31] &= 248
...
func mockUpSigList(curve *TwistedEdwardsCurve, i int) []*SignatureVerParams {
	r := rand.New(rand.NewSource(54321))

	// fake a list of secret keys
	privKeyList := make([]*PrivateKey, i, i)
	for j := 0; j < i; j++ {
		for {
			bIn := new([32]byte)
			...
			// validate secret key
			if pks == nil {
				// invalid secret key, then loop back
				continue
			}
			privKeyList[j] = pks
			r.Seed(int64(j) + 54321)
			// got a ok secret key, then break
			break
		}
	}

	// fake a list of messages to sign
	...
	// sign messages with secret keys to generate signatures
	...
	// fill (pubkey,msg,sig) in a list container
