...
type HashFunc func() (hash *chainhash.Hash, height int64, keyHeight int64, err error)
...
// Peer provides a basic concurrent safe hypercash peer for handling hypercash
...
// callbacks for handling hypercash protocol messages.
//
...
// UpdateLastKeyBlockHeight updates the last known key block for the peer
func (p *Peer) UpdateLastKeyBlockHeight(newKeyHeight int64) {
	p.statsMtx.Lock()
	log.Tracef("Updating last key block height of peer %v from %v to %v",
		p.addr, p.lastKeyBlock, newKeyHeight)
	p.lastKeyBlock = newKeyHeight
	p.statsMtx.Unlock()
}
...
// LastBlock returns the last key block of the peer
func (p *Peer) LastKeyBlock() int64 {
	p.statsMtx.RLock()
	lastKeyBlock := p.lastKeyBlock
	p.statsMtx.RUnlock()
...
func (p *Peer) localVersionMsg() (*wire.MsgVersion, error) {
	var blockNum int64
	var keyBlockNum int64
	if p.cfg.NewestBlock != nil {
		var err error
		_, blockNum, keyBlockNum, err = p.cfg.NewestBlock()
		if err != nil {
			return nil, err
		}
	}
	...
	// Version message.
	msg := wire.NewMsgVersion(ourNA, theirNA, nonce, int32(blockNum), int32(keyBlockNum))
...
func (p *Peer) handleRemoteVersionMsg(msg *wire.MsgVersion) error {
	...
	p.lastKeyBlock = int64(msg.LastKeyBlock)
...
// newPeerBase returns a new base hypercash peer based on the inbound flag.  This
...
// NewInboundPeer returns a new inbound hypercash peer. Use Start to begin
// processing incoming and outgoing messages.
func NewInboundPeer(cfg *Config) *Peer {
	return newPeerBase(cfg, true)
}

// NewOutboundPeer returns a new outbound hypercash peer.
