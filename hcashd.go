...
import (
	...
	"github.com/HcashOrg/hcashd/limits"
)

var cfg *config

// winServiceMain is only invoked on Windows.  It detects when hcashd is running
// as a service and reacts accordingly.
var winServiceMain func() (bool, error)

// hcashdMain is the real main function for hcashd.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func hcashdMain(serverChan chan<- *server) error {
	...
	defer hcashdLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	hcashdLog.Infof("Version %s (Go version %s)", version(), runtime.Version())
	hcashdLog.Infof("Home dir: %s", cfg.HomeDir)

	// Enable http profiling server if requested.
	if cfg.Profile != "" {
		go func() {
			listenAddr := net.JoinHostPort("", cfg.Profile)
			hcashdLog.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			hcashdLog.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			hcashdLog.Errorf("Unable to create memory profile: %v", err)
			return err
		}
	...
	db, err := loadBlockDB()
	if err != nil {
		hcashdLog.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventDBOpen)
		hcashdLog.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		hcashdLog.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		hcashdLog.Infof("Gracefully shutting down the server...")
		server.Stop()
