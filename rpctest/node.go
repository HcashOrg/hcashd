...
import (
	...
	rpc "github.com/HcashOrg/hcashrpcclient"
	"github.com/HcashOrg/hcashutil"
)

// nodeConfig contains all the args, and data required to launch a hcashd process
...
// arguments returns an array of arguments that be used to launch the hcashd
// process.
...
// command returns the exec.Cmd which will be used to start the hcashd process.
...
// to the hcashd process that is launched via Start().
...
// as the base for the log and data directories for hcashd.
...
// start creates a new hcashd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "hcashd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running hcashd process process, and waits until it exits
...
// shutdown terminates the running hcashd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := hcashutil.NewTLSCertPair(elliptic.P521(), org,
		validUntil, nil)
	if err != nil {
		return err
	}
