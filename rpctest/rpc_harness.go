...
// Harness fully encapsulates an active hcashd process to provide a unified
// platform for creating rpc driven integration tests involving hcashd. The
// active hcashd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active hcashd process is fully
...
type Harness struct {
	...
	Node     *hcashrpcclient.Client
	node     *node
	handlers *hcashrpcclient.NotificationHandlers
...
func New(activeNet *chaincfg.Params, handlers *hcashrpcclient.NotificationHandlers, extraArgs []string) (*Harness, error) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the hcashd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []hcashutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure hcashd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created hcashd
...
func (h *Harness) connectRPCClient() error {
	var client *hcashrpcclient.Client
	var err error

	rpcConf := h.node.config.rpcConnConfig()
	for i := 0; i < h.maxConnRetries; i++ {
		if client, err = hcashrpcclient.New(&rpcConf, h.handlers); err != nil {
...
func (h *Harness) NewAddress() (hcashutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() hcashutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut, feeRate hcashutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut, feeRate hcashutil.Amount) (*wire.MsgTx, error) {
...
// harness instance.
func (h *Harness) RPCConfig() hcashrpcclient.ConnConfig {
