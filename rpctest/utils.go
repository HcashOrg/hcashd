...
func syncMempools(nodes []*Harness) error {
	...
	for !poolsMatch {
	retry:
		firstPool, err := nodes[0].Node.GetRawMempool(hcashjson.GRMAll)
		if err != nil {
			return err
		}
		...
		for _, node := range nodes[1:] {
			nodePool, err := node.Node.GetRawMempool(hcashjson.GRMAll)
			if err != nil {
				return err
			}
...
func ConnectNode(from *Harness, to *Harness) error {
	...
	if err := from.Node.AddNode(targetAddr, hcashrpcclient.ANAdd); err != nil {
		return err
	}
