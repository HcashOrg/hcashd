...
type memWallet struct {
	...
	coinbaseAddr hcashutil.Address
	...
	addrs map[uint32]hcashutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]hcashutil.Address)
...
// SetRPCClient saves the passed rpc connection to hcashd as the wallet's
// personal rpc connection.
func (m *memWallet) SetRPCClient(rpcClient *hcashrpcclient.Client) {
	m.rpc = rpcClient
}
...
func (m *memWallet) IngestBlock(header []byte, filteredTxns [][]byte) {
	...
	txns := make([]*hcashutil.Tx, 0, len(filteredTxns))
	for _, txBytes := range filteredTxns {
		tx, err := hcashutil.NewTxFromBytes(txBytes)
		if err != nil {
			panic(err)
		}
		txns = append(txns, tx)
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash, isCoinbase bool, undo *undoEntry) {
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          hcashutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (hcashutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []hcashutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt hcashutil.Amount, feeRate hcashutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := hcashutil.Amount(txSize * int(feeRate))
...
// in atoms-per-byte.
func (m *memWallet) SendOutputs(outputs []*wire.TxOut, feeRate hcashutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate hcashutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt hcashutil.Amount
	for _, output := range outputs {
		outputAmt += hcashutil.Amount(output.Value)
		tx.AddTxOut(output)
	}

	// Attempt to fund the transaction with spendable utxos.
	if err := m.fundTx(tx, outputAmt, hcashutil.Amount(feeRate)); err != nil {
		return nil, err
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() hcashutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance hcashutil.Amount
...
func keyToAddr(key chainec.PrivateKey, net *chaincfg.Params) (hcashutil.Address, error) {
	pubKey := chainec.Secp256k1.NewPublicKey(key.Public())
	serializedKey := pubKey.SerializeCompressed()
	pubKeyAddr, err := hcashutil.NewAddressSecpPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
