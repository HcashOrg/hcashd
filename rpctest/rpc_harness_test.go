...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt hcashutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(hcashutil.Amount(5 * hcashutil.AtomsPerCoin))
	...
	txid = genSpend(hcashutil.Amount(5000 * hcashutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(hcashjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(hcashjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := hcashutil.Amount(5 * 300 * hcashutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = hcashutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := hcashutil.Amount(50 * hcashutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := hcashutil.Amount(numMatureOutputs * 300 * hcashutil.AtomsPerCoin)
