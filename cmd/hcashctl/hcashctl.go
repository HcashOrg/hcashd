...
func main() {
	...
	method := args[0]
	usageFlags, err := hcashjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := hcashjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// hcashjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(hcashjson.Error); ok {
		...
		// The error is not a hcashjson.Error and this really should not
